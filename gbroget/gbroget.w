...
인자 |minDistance|가 1 이상이면 이 자기 고리도 함께 떨어져 나가므로, 거리가 3 미만인
...
@ 함수 |Roget(n, minDistance, prob, seed, dir)|은 |dir| 디렉터리의 \.{roget.dat}에
담긴 정보로 그래프를 짓는다. 만든 그래프의 정점 수는 $\min(n,1022)$이되,
|n=0|이면 기본값 |n=1022|를 쓴다. 인자 |n|이 1022보다 작으면 |n|개의 범주를 무작위로
...
조건 |prob>0|이면, 원래 같으면 들어갔을 호를 |prob/65536|의 확률로 물리친다.
성긴 그래프를 얻는 방법이다.

정점은 무작위 차례로 나타난다. 다만 GraphBase의 모든 ``무작위성''은 재현
가능하다---오직 |seed| 값에만 달렸고, |seed|는 0 이상 $2^{31}$ 미만의 정수면
무엇이든 좋다. 호출 |Roget(1000,3,32768,50)|을 청하는 사람은 누구나, 어느
컴퓨터에서든 똑같은 그래프를 얻는다. 인자 |prob|을 바꾸어도 정점의 선택이나 차례는
그대로이고 호만 달라진다.
...
@ 함수 |Roget|의 뼈대는 \CEE/ 원본을 그대로 따른다: 난수를 앉히고, 정점 수를
...
@ 첫 번째 할 일은 정점 |n|개를 무작위로 골라 뒤섞는 것이다. 표 |mapping|을
꾸려, 정확히 |n|개의 무작위 범주 번호 |k|에 대해서만 |mapping[k]|가 |nil|이
아니게 한다. 그리고 그 |nil| 아닌 값들이 그래프 정점의 무작위 순열이 되게 한다.

배열 |cats|는 아직 쓰이지 않은 범주 번호들의 표다. 변수 |pool|은 |mapping| 값이 아직
...
@ 콜론 뒤의 번호들을 차례로 읽는다. 조건 |j==0|이면 이 범주에는 호가 하나도 없다.
...
@ 여기서 |prob|이 왜 ``65536분의 몇''인지가 드러난다. 함수 |Next|는 0 이상
$2^{31}$ 미만의 난수를 고르게 내놓으므로, 이것을 15비트 오른쪽으로 밀면 0 이상
$2^{16}=65536$ 미만의 고른 난수가 된다. 그 값이 |prob| 미만일 때 호를 물리치니,
물리칠 확률이 정확히 |prob|$/65536$이다. 인자 |prob|$=32768$이면 반, |prob|$=0$이면
하나도 물리치지 않는다.

인자 |prob|의 관문은 \CEE/의 짧은회로 그대로다: 난수는 |mapping[j]|와 거리 조건을
