...
@ 조건 |omit=1|이면 \.{Users} 정점이 그래프에서 빠지고, 따라서 방금 말한 음의 흐름
호가 모두 사라진다. 조건 |omit=2|이면 \.{Adjustments} 정점까지 빠져, 산업 사이의
흐름만 보여 주는 79개 부문이 남는다. (물론 |omit>0|이면 그래프는 더 이상
``순환''이 아니다.) 두 특별 정점이 남으면 \.{Users}가 마지막, \.{Adjustments}가
그 앞 정점이다.

@ 조건 |threshold=0|이면 흐름이 0이 아닌 호가 모두 생긴다. 조건 |threshold>0|이면 그래프가
...
조건 |n=79|, |omit=2|, |threshold=0|이면 유향 그래프의 호는 가능한
$79\times79=6241$개 가운데 4602개다. 인자 |threshold|를 1로 올리면 4473개로 줄고,
6000으로 올리면 겨우 72개만 남는다.

@ 그래프의 정점 수는 $\min(n,81-|omit|)$이다. 인자 |n|이 |81-omit|보다 작으면,
...
이름은 \.{Goods}(재화)와 \.{Services}(용역)가 된다. 조건 |n=3|이면 \.{Goods}가
...
@ 조건 |seed=0|이면 트리 구조를 지키는 한도 안에서 |n|개 부문의 총투입과 총산출이
되도록 고르게 나뉘도록 가지친다. 조건 |seed>0|이면 무작위로 가지치되, 원 트리의 모든
|n|-잎 부분트리가 (기계에 무관한 방식으로) 거의 같은 확률로 나오도록 한다.
인자 |seed| 값은 1부터 $2^{31}-1=2147483647$까지면 무엇이든 좋다.
...
@ 타입 |builder|는 \CEE/의 정적 전역들(스택·노드 블록·색인표)을 담아 패키지 수준
가변 상태를 피한다.
@<타입...@>=
type builder struct {
	rng             *gbflip.RNG
...
@ 함수 |left|는 노드 |p|의 왼쪽 자식(전위 순회에서 바로 다음 노드 |p+1|)을 준다.
여러 곳에서 쓰인다.
...
@ 함수 |Econ|의 뼈대는 \CEE/ 원본을 그대로 따른다: 난수를 앉히고, 매개변수를
...
@ \.{econ.dat}의 나머지는 $81\times80$ 행렬이다. 그중 |k|번째 행은 부문 |k|가
...
@ 조건 |seed=0|일 때는 먼저 트리를 아래에서 위로 훑어 각 거시 부문의 총투입을
...
노드 |special|(\.{Users})는 두 구실을 한다: |total|의 내림차순으로 정렬된
...
@ 주어진 트리의 |l|-잎 부분트리를 고르게 뽑는 방법은 이렇다. 조건 |l=1|이면 뿌리를
고르면 그만이다. 조건 |l>1|이면 다음 생각을 쓴다: 주어진 트리 $T$가 부분트리 $T_0$과
$T_1$을 갖는다고 하자. 그러면 $T$의 |l|-잎 부분트리 수는
$$T(l)=\sum_k T_0(k)T_1(l-k)$$
이다. 난수 $r$을 $0$과 $T(l)-1$ 사이에서 뽑아, $\sum_{k\le m}T_0(k)T_1(l-k)>r$인
가장 작은 $m$을 찾는다. 그러고는 $T_0$의 $m$-잎 부분트리와 $T_1$의 $(l-m)$-잎
부분트리를 재귀적으로 구한다.

@ 수 $T(l)$이 $2^{31}$ 이상이면 곤란해진다. 하지만 그럴 때는 위 식의 $T_0(k)$와
$T_1(l-k)$를 각각 $\lceil T_0(k)/d_0\rceil$과 $\lceil T_1(k)/d_1\rceil$로
바꾸면 된다. 두 값 $d_0$과 $d_1$은 아무 상수나 좋다. 이러면 $T(l)$이 작아지면서도
$k$의 분포는 거의 그대로다.
...
@ 함숫값 $T(l)$을 아래에서 위로 계산해 |table|에 담고(노드 |p|가 잎이 아니면
|p.table[0]|은 그 아래 잎의 수, |p.table[l]|은 $1\le l\le|p.table[0]|$에 대한
$T(l)$), 무작위 트리를 위에서 아래로 기른다.

필드 |tag|에는 그 노드를 뿌리로 하는 부분트리에서 기를 잎의 수를 담는다. 이
...
@ 여기서 하는 일은 본질적으로 두 생성 함수를 곱하는 것이다. 생성 함수 $f(z)=\sum_l
...
노드 |p|가 |pl|과 |pr|의 합집합으로 만들어진다고 하자. 그러면 |p|에서 나가는 호의
...
표 |nodeIndex|는 |nil|이 아닌 자리가 지금 살아 있는 노드 전부가 되도록 계속
간수한다. 노드 |pl|과 |pr|이 |p|에 밀려 가지치기될 때, |p|는 |nodeIndex| 안에서
...
@ 노드 |p|는 |pl|의 |node_index| 자리를 물려받고, |pr|의 자리는 |nil|이 된다.
...
인자 |threshold|가 0이면 문턱을 $-99999999$로 두어, 음의 흐름을 가진 호까지 모두
살아남게 한다.
...
@ 호출 |econ(40,0,400,-111)|은 정점 40개·호 512개짜리 그래프이고, 그 11번 정점은
...
@ 문턱값과 가지치기의 발표된 값들이다. 호출 |Econ(79,2,0,0)|은 가능한 6241개
...
@ 조건 |n=3|이면 갈리는 쪽은 \.{Services}여서 \.{Goods}·\.{Indirect services}·
...
@ 조건 |n=80|, |omit=1|이면 SIC 목록의 길이가 모두 1이라야 한다. 그리고 종이 관련
