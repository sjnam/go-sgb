...
func vIdx(g *gbgraph.Graph, v *gbgraph.Vertex) int64 {
...
func vMap(v *gbgraph.Vertex, src, dst *gbgraph.Graph) *gbgraph.Vertex {
	i := vIdx(src, v)
...
func inVertexSlice(v *gbgraph.Vertex, vertices []gbgraph.Vertex) bool {
	return gbgraph.VertexIn(v, vertices)
}
...
func getTmp(v *gbgraph.Vertex) *gbgraph.Vertex  { p, _ := v.U.(*gbgraph.Vertex); return p }
func setTmp(v, u *gbgraph.Vertex)               { v.U = u }
func getTlen(v *gbgraph.Vertex) *gbgraph.Arc    { p, _ := v.Z.(*gbgraph.Arc); return p }
func setTlen(v *gbgraph.Vertex, a *gbgraph.Arc) { v.Z = a }
func getMult(v *gbgraph.Vertex) int64           { i, _ := v.V.(int64); return i }
func setMult(v *gbgraph.Vertex, i int64)        { v.V = i }
func getMinlen(v *gbgraph.Vertex) int64         { i, _ := v.W.(int64); return i }
func setMinlen(v *gbgraph.Vertex, i int64)      { v.W = i }
func getVmap(v *gbgraph.Vertex) *gbgraph.Vertex { p, _ := v.Z.(*gbgraph.Vertex); return p }
func setVmap(v, u *gbgraph.Vertex)              { v.Z = u }
func getInd(v *gbgraph.Vertex) int64            { i, _ := v.Z.(int64); return i }
func setInd(v *gbgraph.Vertex, i int64)         { v.Z = i }
func getSubst(v *gbgraph.Vertex) *gbgraph.Graph { g, _ := v.Y.(*gbgraph.Graph); return g }
...
func SetInd(v *gbgraph.Vertex, i int64) { setInd(v, i) }

// SetSubst sets the substitution graph for v (utility field y.G).
func SetSubst(v *gbgraph.Vertex, g *gbgraph.Graph) { v.Y = g }
...
func Subsets(n, n0, n1, n2, n3, n4, sizeBits int64, directed bool) (*gbgraph.Graph, error) {
	var st basicState
	d, k, periodic := st.normalizeSimplex(n, n0, n1, n2, n3, n4)
	if periodic {
		if d > maxD {
...
func Complement(g *gbgraph.Graph, copy, self, directed bool) (*gbgraph.Graph, error) {
	if g == nil {
...
func Gunion(g, gg *gbgraph.Graph, multi, directed bool) (*gbgraph.Graph, error) {
	...
	for vi := range n {
		...
		var vvv *gbgraph.Vertex
		if vi < gg.N {
			vvv = &gg.Vertices[vi]
		}

		insertUnion := func(src *gbgraph.Vertex, a *gbgraph.Arc) {
...
func Lines(g *gbgraph.Graph, directed bool) (*gbgraph.Graph, error) {
	...
	newG := gbgraph.NewGraph(m)
	if newG == nil {
		return nil, gbgraph.ErrNoRoom
	}
	gbgraph.MakeCompoundID(newG, "lines(", g, fmt.Sprintf(",%d)", boolInt(directed)))
	...
	panicRestore := func() (*gbgraph.Graph, error) {
		var vOrig *gbgraph.Vertex
		for i := int64(0); i < ui; i++ {
			u := &newG.Vertices[i]
			uuV, _ := u.U.(*gbgraph.Vertex)
			if uuV != vOrig {
				vOrig = uuV
				vOrig.Z = u.Z
				u.Z = nil
	...
	if directed {
		for i := int64(0); i < m; i++ {
			u := &newG.Vertices[i]
			v, _ := u.V.(*gbgraph.Vertex) // second endpoint in g
			if v == nil || v.Arcs == nil {
				continue
			}
		...
		for i := int64(0); i < m; i++ {
			u := &newG.Vertices[i]
			v, _ := u.U.(*gbgraph.Vertex) // first endpoint
...
func Product(g, gg *gbgraph.Graph, typ int64, directed bool) (*gbgraph.Graph, error) {
	...
	typStr := (typ&2 - (typ & 1))
	gbgraph.MakeDoubleCompoundID(newG, "product(", g, ",", gg,
...
func Induced(g *gbgraph.Graph, description string, self, multi, directed bool) (*gbgraph.Graph, error) {
	...
	gbgraph.MakeCompoundID(newG, "induced(", g,
