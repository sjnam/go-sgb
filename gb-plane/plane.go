...
func zCoord(v *gbgraph.Vertex) int64 { i, _ := v.Z.(int64); return i }

// XCoord returns the x coordinate stored in vertex v.
func XCoord(v *gbgraph.Vertex) int64 { return xCoord(v) }

// YCoord returns the y coordinate stored in vertex v.
func YCoord(v *gbgraph.Vertex) int64 { return yCoord(v) }

// ZCoord returns the z coordinate (unique ID) stored in vertex v.
func ZCoord(v *gbgraph.Vertex) int64 { return zCoord(v) }
...
func ffFunc(t, u, v, w *gbgraph.Vertex) int64 {
...
func ggFunc(t, u, v, w *gbgraph.Vertex) int64 {
	wx, wy := xCoord(w), yCoord(w)
	tx, ty := xCoord(t)-wx, yCoord(t)-wy
	ux, uy := xCoord(u)-wx, yCoord(u)-wy
	vx, vy := xCoord(v)-wx, yCoord(v)-wy
...
func hhFunc(t, u, v, w *gbgraph.Vertex) int64 {
	return (xCoord(u) - xCoord(t)) * (yCoord(v) - yCoord(w))
}

func jjFunc(t, u, v, w *gbgraph.Vertex) int64 {
...
func dlFlip(c, d, e *dlArc, tpp, p *gbgraph.Vertex, xp, xpp *dlNode) {
	ep := e.next
	cp := c.next
	cpp := cp.next
	e.next = c
...
func Delaunay(g *gbgraph.Graph, f func(*gbgraph.Vertex, *gbgraph.Vertex)) {
	if g.N < 2 {
		return
	}
...
func Plane(n, xRange, yRange int64, extend bool, prob, seed int64) (*gbgraph.Graph, error) {
	...
	Delaunay(g, func(u, v *gbgraph.Vertex) {
		if rng.Next()>>15 >= prob {
			if u != nil {
				if v != nil {
...
func PlaneMiles(n, northWeight, westWeight, popWeight int64, extend bool, prob, seed int64) (*gbgraph.Graph, error) {
