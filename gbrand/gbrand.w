...
그래프를 짓는데, 중복도 자기 고리도 없다.

정점 1000개 위에 놓일 수 있는 무향 간선은 ${1000\choose2}=499500$가지이므로,
이 명세를 만족하는 그래프는 정확히 $499500\choose5000$가지다. 난수의 원천이
이상적이라면 그 하나하나가 똑같이 나올 법하다. 씨앗을 바꾸면 다른 그래프가
나오지만, 같은 씨앗이면 세상 어디서 돌려도 같은 그래프가 나온다---그래서
그래프 알고리즘을 저마다 다른 곳에서 실험하는 연구자들이 똑같은 조건에서
견줄 수 있다. 씨앗은 $0\le s<2^{31}$인 정수면 무엇이든 좋다.

@ 자기 고리와 중복 호를 허용하고 정점에 고르게 분포하는 방향 그래프를 얻으려면
$$\hbox{|RandomGraph(n, m, 1, true, true, nil, nil, 1, 1, s)|}$$
라고 하면 된다. 이 유향 그래프의 호 $m$개는 저마다, 모든 $u$와 $v$에 대해
$u$에서 $v$로 갈 확률이 $1/n^2$이다. 자기 고리를 금하면(|true, true|를
|false, true|로 바꾸면) 모든 $u\ne v$에 대해 그 확률이 $1/(n^2-n)$이 된다.

@ 정점 $k$가 $k+1$보다 호의 출발점이 될 확률은 두 배이고 도착점이 될 확률은
절반인 방향 그래프를 얻고 싶다면
$$\hbox{|RandomGraph(31, m, 1, true, true, d0, d1, 0, 255, s)|}$$
을 써 보라. 여기서 두 배열은
$$\vbox{\halign{#\hfil\cr
|d0 := []int64{0x20000000, 0x10000000, |\dots|, 4, 2, 1, 1}|\cr
|d1 := []int64{1, 1, 2, 4, |\dots|, 0x10000000, 0x20000000}|\cr}}$$
이다(둘 다 원소가 31개이고 합이 $2^{30}$이다). 그러면 호의 약 1/4이 0에서 30으로
가고, 30에서 0으로 가는 호는 지극히 드물다---확률이 $2^{-60}$이다. 덧붙여, 이
보기에서 호의 길이는 |minLen=0|, |maxLen=255|이므로 0과 255 사이에 고르게
분포하는 무작위 바이트가 된다.

@ 이 보기에서 |multi|를 0으로 두어 중복 호를 금하면, 출발점과 도착점이 앞선
호와 같은 호는 길이에 상관없이 모두 버려진다. 그런 경우 |m|이 너무 크지
...
@ \CEE/ 원본은 어긋난 자리마다 서로 다른 임시 수를 돌려주었다. 우리 판은
그러지 않고 오류를 |gbgraph.PanicCode| 하나로 통일한다. 그편이 나머지 모듈과
어울리기 때문이다.
...
@ 들어가며 절의 31정점 보기를 그대로 세워, 거기 적은 세 가지 주장을 확인한다:
호의 약 1/4이 $0\to30$이고, $30\to0$은 확률 $2^{-60}$이라 사실상 나타나지
않으며, 길이는 $[0,255]$의 무작위 바이트다.
