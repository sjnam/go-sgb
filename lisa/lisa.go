...
func FirstPixel(v *graph.Vertex) int64 { i, _ := v.Y.(int64); return i }
func LastPixel(v *graph.Vertex) int64  { i, _ := v.Z.(int64); return i }

// Graph utility-field accessors for PlaneLisa.
func MatrixRows(g *graph.Graph) int64 { i, _ := g.UU.(int64); return i }
func MatrixCols(g *graph.Graph) int64 { i, _ := g.VV.(int64); return i }

// PixelVal returns the scaled pixel value stored in an arc of a BiLisa graph.
func PixelVal(a *graph.Arc) int64 { i, _ := a.B.(int64); return i }
...
func adjac(g *graph.Graph, u, v *graph.Vertex) {
