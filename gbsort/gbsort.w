...
것이다. 예컨대 {\sc GB\_\,WORDS}의 생성기
|Words(n, wtVector, wtThreshold, seed, dir)|는 영어에서 가장 흔한 다섯 글자
단어 |n|개로 그래프를 만드는데, 흔한 정도는 주어진 무게 벡터 |wtVector|가
정한다. 무게가 같은 단어가 여럿이면 그중에서 무작위로 고르고 싶다 — 무게
벡터가 모든 단어에 같은 무게를 준다면 완전히 무작위한 단어 선택을 얻는다는
뜻이기도 하다.

함수 |LinkSort|가 바로 이 일에 맞춤한 연장이다. 노드들의 링크드 리스트를 받아
...
원본이 \.{gb\_sort.w} 안에 선언한 {\bf node} 타입은 그 규약을 만족하는 가장
단순한 본보기일 뿐이고, 실제로 정렬되는 것은 대개 훨씬 살진 레코드였다.

이 재주에는 대가가 따랐다. 노드의 타입이 응용마다 다르니, 헤더 파일
\.{gb\_sort.h}는 결과 배열을 타입 없는
|extern char| $*gb\_sorted[];$
로 내놓을 수밖에 없었고, 쓰는 쪽이 매번 제 타입으로 도로 캐스팅해야 했다.
\GO/에는 그런 캐스팅이 없으므로, 노드를 제네릭 타입 |Node[T]|로 만들어
딸린 데이터를 |Data| 필드에 담는다. 이러면 캐스팅도, typed-|nil| 함정도
없고, 결과 배열도 |char*|가 아니라 제 타입 그대로 나온다.

정렬은 |Key| 필드로 하며, 키는 저마다 $2^{31}$보다 작은 음이 아닌
정수라야 한다.

@ 결과를 두는 자리도 달라졌다. 원본은 정렬 결과를 전역 배열 |gb_sorted|에
남기고 |gb_linksort|는 아무것도 돌려주지 않았지만, 우리는 패키지 수준의
가변 상태를 두지 않기로 했으므로 |LinkSort|가 128개 리스트를 값으로
돌려준다. 덕분에 여러 고루틴이 같은 |LinkSort|를 겹쳐 불러도 서로의 결과를
밟지 않는다.
...
@ 타입 |Node|의 |Data| 필드가 원본의 ``뒤따르는 임의의 필드들''을 대신한다.
...
원본의 여섯 조각은 저마다 ``목적지 리스트를 모두 비운다''는 루프로 시작했다.
갈 곳이 될 256개 포인터를 매번 |nil|로 되돌려 놓아야 했기 때문이다. 우리는
통 배열을 이름 붙은 반환값 |b|로 새로 선언하므로 \GO/가 알아서 |nil|로 채워
준다 — 그 준비 루프 여섯 벌이 통째로 사라졌다.
...
좌우한다. 인자 |forward|가 참이면 통을 0에서 255로, 아니면 255에서 0으로 읽는다.
원본의 주석대로, ``까다롭지만 잘 된다.''
...
충분하다. Knuth는 여기에 한마디를 덧붙였다---부호 있는 정수나 부호 없는
|long|을 정렬하는 비슷한 루틴도 짤 수야 있지만, 그러면 \CEE/ 코드가 이식성을
잃는다고. 타입 |long|의 폭이 기계마다 달랐던 시절의 근심이다. 타입 |int64|의 폭이 언어
명세로 못 박혀 있는 \GO/에는 그 걱정이 없지만, 우리는 원본과 같은 규약을 지켜
128개 통에서 멈춘다.
