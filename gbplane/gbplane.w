...
@ 함수 |Plane(n, xRange, yRange, extend, prob, seed)|는 정수 좌표가
$$\{\,(x,y)\mid 0\le x<|xRange|,\ 0\le y<|yRange|\,\}$$
에 고르게 흩어진 정점을 가진 평면 그래프를 짓는다. 인자 |xRange|·|yRange|는 많아야
$2^{14}=16384$이고, 0으로 주면 그 기본값이 쓰인다. 인자 |extend|가 참이면 정점이
...
두 점의 유클리드 거리에 $2^{10}$을 곱해 반올림한 값이다. 인자 |extend|이면 $\infty$와
볼록 껍질의 모든 점 사이 간선(버려지지 않으면 길이 |infty|$=2^{28}$)도
삼각분할에 든다.

@ 인자 |extend|가 참이고 |prob|이 0이면, 그래프는 정점 $n+1$개와 간선 $3(n-1)$개를
...
나오되 그 방식은 기계에 무관해 여러 컴퓨터에서 똑같이 재현된다. 인자 |seed|는 0과
$2^{31}-1$ 사이면 무엇이든 좋다.

@ 유틸리티 필드 |x_coord|·|y_coord|·|z_coord|는 각각 |X.I|·|Y.I|·|Z.I|다.
함수 |Plane|은 좌표를 여기에 두고, |z_coord|에는 |Delaunay|가 퇴화(점이 같거나
...
@ 필드 |z_coord|에는 무작위 ID를 둔다. \CEE/처럼 |gb_next_rand()/n*n+k| 꼴이라 정점마다
...
@ 함수 |Delaunay|가 부를 콜백은 유한 간선에 유클리드 길이를, 무한 간선에 |infty|를
...
간선은 정점을 그 ``이웃''과 잇는다. 함수 |Delaunay|는 삼각분할을 그래프로 돌려주는
대신, 델로네 간선으로 이어진 정점 쌍 |u|, |v|마다 콜백 |f(u,v)|를 부른다. 정점 |u|나
...
@ 먼저 정확한 결과를 위한 산술 서브루틴부터 다진다. 타입 |long|이 $2^{31}$ 미만이라고
본다. 함수 |intSqrt(x)|는 $\lfloor2^{10}\sqrt x+{1\over2}\rfloor$, 곧 음 아닌 정수
...
같은 말을 달리 할 수도 있다. 선분 $uv$가 델로네 간선일 필요충분조건은 $u$와 $v$의
...
이제 이 배치에 $(0,0,1)$을 더하자. 값 $r$이 무한대로 갈 때 구면 위의 극한점이다.
...
@ 함수 |Delaunay|는 주어진 정점 집합의 델로네 삼각분할을 찾는다. 이 모듈이
...
$2^{14}=16384$보다 작아야 한다. 필드 |z_coord|에는 정점마다 다른 유일한 ID 번호가
들어 있어야 한다. 알고리즘이 퇴화한 경우의 순위를 가르는 데 쓰기 때문이다.
(유의하라: 입력 자료에 대한 이 가정들은 부르는 쪽의 책임이다. 함수 |Delaunay|는
그것을 두 번 확인하지 않는다. 가정이 깨지면 파국적인 실패가 일어날 수 있다.)

@ 함수 |Delaunay|는 삼각분할을 그래프로 돌려주지 않는다. 대신 델로네 간선으로
이어지는 정점 쌍 |u|·|v|마다 매개변수로 받은 함수 |f(u,v)|를 불러, 답을
암묵적으로 알린다. 정점 |u|와 |v|는 정점을 가리키거나 |nil|인데, |nil|은 정점
...
가까운 정수를 구하는 루틴이다. 조건 |x>0|이면 이것은
...
함수 |signTest(x1,x2,x3,y1,y2,y3)|은 내적
...
@ 부등식 $|c|<2^{29}$임을 쓴다. \CEE/의 두 |goto ez|는 |a==0|일 때로 가는데, |a!=0|
...
@* 행렬식. 함수 |Delaunay|가 내리는 모든 판단은 두 기하 술어에 달려 있고, 그 둘은
...
술어 |ccw(u,v,w)|가 성립할 필요충분조건은 행렬식
...
(mate)\/})로 나타낸다. 타입 |darc|는 |gbgraph.Arc|와 다른 것으로, 세 필드가 있다:
|vert|는 이 호가 가는 정점(또는 $\infty$면 |nil|), |next|는 같은 삼각형을 왼쪽에
둔 다음 호, |inst|는 삼각형이 바뀔 때 고칠 분기 노드다. 등식 |p.next.next.next==p|이고
...
@ 함수 |flip|은 호 |c|의 왼쪽·오른쪽 두 삼각형을, 끝 노드 |xp|·|xpp|에 대응하는 두
삼각형으로 갈아 끼운다. 변수 |t|·|tp|는 실제로 안 쓰인다.
...
@ 점 |p|를 품은 삼각형의 꼭짓점을 반시계 방향으로 |q|, |r|, |s|라 하자. 끝 노드
|x|를 고쳐, 앞으로 이 삼각형의 점을 세 부분 삼각형 가운데 하나에서 찾도록 한다.
꼭짓점 |q|가 $\infty$면 삼각형 대신 ``쐐기(wedge)''를 쓴다.
...
@ 값 |q=a.vert|가 |nil|일 수 있음이 유일한 미묘함이다. 끝 노드는 무한 삼각형의
알맞은 호를 가리켜야 한다.
...
@* 마일리지 자료 이용. 함수 |PlaneMiles(n, northWeight, westWeight, popWeight, extend,
...
인자 |maxDistance|를 1로 주어 |Miles|가 정점만 있고 간선은 없는 그래프를 내게 한다.
...
@ \CEE/ 생성기들은 전역 |gb_flip| 스트림 하나를 공유한다. 함수 |PlaneMiles|는 |rng|
...
\.{sample.correct}와 대조한다. 함수 |PlaneMiles|는 |Delaunay|를 end-to-end로 거치므로
가장 든든한 검사다.
...
@ 호출 |plane_miles(50,500,-100,1,1,40000,271818)|은 정점 51개·호 96개짜리
...
@ 함수 |Plane|은 \.{sample.correct}에 없으므로 짜임새를 확인한다. 조건 |extend|이고 |prob=0|
...
@ 조건 |prob=32768|이면 델로네 간선의 절반쯤이 버려지므로 평균 차수가 3 언저리가
