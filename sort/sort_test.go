...
func TestDecreasingOrder(t *testing.T) {
	...
	sorted := LinksSort[emptyData](buildList(keys), rng)
...
func TestEqualKeysRandomOrder(t *testing.T) {
	...
	rng1 := flip.New(1)
	sorted1 := LinksSort[emptyData](buildList(keys), rng1)
	var order1 []int64
	for j := 127; j >= 0; j-- {
	...
	rng2 := flip.New(2) // different seed
	sorted2 := LinksSort[emptyData](buildList(keys), rng2)
	var order2 []int64
	for j := 127; j >= 0; j-- {
...
func TestWithEmbeddedData(t *testing.T) {
	...
	sorted := LinksSort[cityData](&nodes[0], rng)
	out := collectSorted(sorted)
