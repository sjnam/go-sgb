...
$\delta$들을 거슬러 훑기 쉽게 해 준다. 변수 |xx|와 |yy|는 각각 이동 전과 이동 후의
좌표값이다. 배열 |sig|만 색인 $d+1$까지 쓰므로 한 칸 더 크게 잡는다.
...
인자 |n1|부터 |n4|까지가 판의 크기를 정한다. 예컨대 $n_1$행 $n_2$열의 2차원 판을
...
0 이하의 값 $n_{k+1}$에서 멈춘다. 조건 $k=0$이면(곧 |n1<=0|이면) 기본 크기
...
@ 인자 |piece|는 말의 이동 규칙이다. 조건 |piece>0|이면 두 자리 |u|와 |v| 사이의
...
유일한 수다. 조건 |piece=2|이면 $(x,y)$에서 $(x\pm1,y\pm1)$로 가는 네 수가 되니, 왕과
비숍이 함께 둘 수 있는 수다(옛 이슬람 체스에서 이 수만 두는 말을 페르스(fers)라
불렀다). 조건 |piece=5|는 $(x,y)$에서 $(x\pm1,y\pm2)$나 $(x\pm2,y\pm1)$로 가는
나이트의 수다. 조건 |piece=3|이면 2차원 판에서는 합법인 수가 하나도 없지만, 3차원에서
$(x,y,z)$에서 $(x\pm1,y\pm1,z\pm1)$로 가는 수가 합법이 된다. 조건 |piece=0|은 기본값
|piece=1|로 바뀐다.

인자 |piece|가 음수이면 $\vert|piece|\vert$에 해당하는 기본 이동을 몇 배로든 이어
...
@ 인자 |wrap|이 0이 아니면, 그 비트가 가리키는 좌표들은 제 크기로 나눈 나머지로
...
|u|에서 |v|로 호가 있는 유향 그래프인 추이 토너먼트다. 호출 |Empty(n)|에 간선이
없는 까닭은 1차원 판에서 거리 $\sqrt2$인 두 자리가 있을 수 없기 때문이다.
호출 |Circuit(n)|과 |Cycle(n)|에서는 |wrap=1|이 판의 양 끝을 이어 붙여 길이 |n|의
무향 회로와 유향 순환을 만든다.
...
@ 함수 |bool|을 표식 문자열용 0/1로 바꾸는 잔심부름이다.
...
@ 이제 조금 까다로운 이동 생성기다. 값 $p=\vert|piece|\vert$로 두고, 바깥
...
$v+\delta$로 가는 이동을 낸다. 인자 |directed|가 거짓이면 |NewArc| 대신 |NewEdge|를
써서 $v+\delta$에서 |v|로 되돌아오는 호까지 함께 만든다.

표 |wr|을 채울 때 \CEE/ 원본은 |wrap|이 음수일 때 오른쪽 시프트가 부호를
...
@ 배열 |sig| 덕에 |p|를 제곱들의 순서 있는 합으로 나누는 모든 방법을 거슬러
훑기 쉽다.
...
@ 인자 |piece|가 음수일 때의 합법적 이동은 감김까지 헤아리면 이렇게 정해진다:
...
@*일반화된 삼각형 판. 함수 |Simplex(n,n0,n1,n2,n3,n4,directed)|은 일반화된
삼각형이나 사면체 모양에 바탕한 그래프를 짓는다. 정신은 |Board|가 만드는 게임판과
비슷하지만, 이쪽은 중국 체커의 판처럼 직사각형이 아닌 격자를 다룬다. 조건 |piece=1|인
...
정점은 이웃을 많아야 6개 갖는다. 조건 $d=3$이면 정점들이 삼각형 층을 쌓아 올린
사면체 배열을 이루어 이웃을 12개까지 가질 수 있다. 일반적으로 $d$차원 단체
배열의 정점은 이웃을 많아야 $d(d+1)$개 갖는다.

인자 |directed|가 참이면 호는 사전순으로 더 큰 이웃으로만 간다---위 삼각형 배열로
...
조건 $k>0$이고 $n_k=0$이면 $d$는 $k-1$이고 좌표는 주어진 $(n_0,\ldots,n_d)$로
갇힌다. 조건 $k>0$이고 $n_k<0$이면 $d=\vert n_k\vert$이고 좌표의 상한은 무한 주기
나열 $(n_0,\ldots,n_{k-1},n_0,\ldots,n_{k-1},n_0,\ldots)$의 앞 $d+1$개를 쓴다.
조건 $k=0$이고 $n_0<0$이면 $d=\vert n_0\vert$이고 좌표에 상한이 없는데, 이는
...
정점 $(x_0,x_1,\ldots,x_5)$을 낸다. 조건 |n0=1|, |n1=-d|는 사실상 $(d+1)$원소 집합
...
@ 함수 |Simplex|는 매개변수를 정규화하고, 가능한 $(x_0,\ldots,x_d)$의 수를 세어
...
그 이름으로 찾을 수 있다. 함수 |Subsets|와 공유하는 정점 생성 골격이되, |Simplex|는
...
|builder|의 메서드로 둔다. 함수 |normalizeSimplex|는 차원 |d|와 좌표 크기
...
에서 $z^n$의 계수다. 다항식 $1+z+\cdots+z^{n_j}$을 곱하는 산뜻한 방법이 있다:
...
값 $y_j=n_j+\cdots+n_d$와 $\sigma_j=n-(x_0+\cdots+x_{j-1})$를 유지하며, 조건
...
@ 함수 |advancePartial|은 조건을 어기지 않고 $x_k$를 키울 수 있는 가장 큰 |k|를
...
@*부분집합 그래프. 함수 |Subsets(n,n0,n1,n2,n3,n4,sizeBits,directed)|은 같은 인자를
...
@ 보기를 들자. 조건 $n=3$이고 $(|n0|,|n1|,|n2|,|n3|)=(2,2,2,0)$이라 하면, 정점은
...
이다. 좌표 $(x_0,x_1,\ldots,x_d)$와 $(y_0,y_1,\ldots,y_d)$가 나타내는 두
다중집합의 교집합은
...
@ 인자 |directed|가 참이면 호는 $u\le v$일 때만 |u|에서 |v|로 간다. 자기 고리는
|sizeBits|의 이진 표현에 $2^n$ 항이 있을 때에만 생기며, 그때는 모든 정점에
고리가 하나씩 달린다(무향 그래프에서는 그런 고리가 호 두 개로 나타난다).

함수 |DisjointSubsets(n,k)|는 정점이 $n\choose k$개이고 서로소인 $k$원소 부분집합끼리
...
@ 함수 |Subsets|는 |Simplex|와 똑같은 논리로 정점을 낳지만, 호는 무차별 대입으로
...
@ 함수 |Simplex|와 같은 생성 골격이되, 이웃은 무차별 대입으로 잇는다.
...
교집합 크기 |ss|를 잰다. 값 |ss|가 |sizeBits|의 한 비트와 맞으면 잇는다.
...
@ 함수 |DisjointSubsets|와 |Petersen| 별명이다.
...
@*순열 그래프. 함수 |Perms(n0,n1,n2,n3,n4,maxInv,directed)|은 다중집합의 순열
...
@ 인자 |n0|부터 |n4|는 |Subsets| 루틴에서와 꼭 같은 방식으로 다중집합의 구성을
...
조건 |n0=-d|이면 모든 중복도 $n_k$가 1이고, |n0>0|이면서 |n1=-d|이면 모든 중복도가
|n0|이 된다. 조건 |n0|과 |n1|이 양수이면서 |n2=-d|이면 중복도가
$(|n0|,|n1|,|n0|,|n1|,\ldots)$로 번갈아 가고, |n0|·|n1|·|n2|가 양수이면서
|n3=-d|이면 주기 나열 $(|n0|,|n1|,|n2|,|n0|,|n1|,\ldots)$의 앞 $d+1$개를 쓴다.
조건 |n4|만 빼고 다 양수이고 |n4=-d|일 때도 마찬가지로 주기적이다.
...
조건 |n0>0|일 때 |Perms(n0,0,0,0,0,maxInv,directed)|를 해 보고 싶지는 않을 것이다.
같은 원소 |n0|개로 이뤄진 다중집합에는 순열이 하나뿐이기 때문이다.

크기 $n$인 집합의 $n!$개 순열을 모두 원하는 특별한 경우는
|AllPerms(n,directed)|로 얻는다.

@ 조건 |maxInv=0|이면 뒤바뀜 수에 상관없이 모든 순열을 쓴다. 그때 그래프의 정점
수는 다항 계수
...
조건 $d=1$인 경우에는 사실상 |n0+n1|개에서 |n1|개를 뽑는 모든 조합을 얻는 셈이다.
1이 놓인 자리가 곧 부분집합의 원소에 대응한다.

인자 |directed|가 참이면 뒤바뀜이 정확히 하나 더 많은 이웃으로 가는 유향 호만
남는다. 이때 그래프는 흥미로운 성질을 지닌 속(lattice)을 이루는 부분 순서에
대응한다. Bennett과 Birkhoff가 {\sl Algebra Universalis\/} {\bf32}(1994),
115--144에 쓴 글을 보라.

@ 함수 |Perms|는 구조가 |Simplex|와 매우 비슷하다. 먼저 다중집합을 정규화하고,
...
@ 함수 |Simplex|의 코드를 빌려 쓰려고 |normalizeSimplex|에 |bufSize|를 |n| 삼아
넘겼으니, 이제 진짜 |n|(다중집합의 크기)과 가능한 뒤바뀜의 최댓값 |ss|를 다시
셈한다. 값 |ss=\sum_{j<k}n_jn_k$다.
...
@ 순열을 낳는 동안 뒤바뀜표 $(y_1,\ldots,y_n)$을 유지한다. 값 $y_k$는 다중집합의
$k$번째 원소를 첫 원소로 하는 뒤바뀜의 수다. 변수 $z$는 뒤바뀜 없는 첫 순열을 담는다.
...
@ 순열 논리의 핵심이다. 값 $y_k$를 정당하게 1 키울 수 있는 가장 큰 |k|를 찾는다.
...
@*분할 그래프. 함수 |Parts(n,maxParts,maxSize,directed)|는 정수 |n|을 최대
|maxParts|개의 부분으로, 각 부분이 |maxSize| 이하가 되게 나누는 방법들을
정점으로 삼는다. 두 분할은 한쪽의 두 부분을 합쳐 다른 쪽을 얻을 수 있을 때
이웃이다. 인자 |maxParts|나 |maxSize|가 0이면 |n|으로 바뀌어 제한이 없어진다.
인자 |n|의 분할 $p(n)$개를 다 원하면 |AllParts(n,directed)|를 부른다.

@ 함수 |Parts|는 |Perms|와 구조가 비슷하되 자기만의 부분해 완성·전진 논리를 쓴다.
...
@ 분할을 낳는 동안 $\sigma_j=n-(x_1+\cdots+x_{j-1})$을 유지한다. 값 $x_0=|maxSize|$,
...
부분으로 쪼개 앞선 이웃을 만든다. 좌표 $(x_1,\ldots,x_{j-1})$은 이미 |nn|에
...
@*이진 트리 그래프. 함수 |Binary(n,maxHeight,directed)|는 내부 노드가 |n|개이고
...
트리에 가하는 이 변환을 흔히 ``회전''이라 부른다). 인자 |directed|가 참이면 유향 호는
...
@ 조건 |maxHeight=0|이면 |n|으로 바뀌는데, 이는 잎의 높이에 아무 제한이 없다는
...
호출 |AllTrees(n,true)|가 만드는 ${2n+1\choose n}/(2n+1)$개 정점의 유향 그래프에
...
@ 함수 |Binary|는 |Parts|와 비슷하나 세부가 더 흥미롭다. 큰 |n|에서는 작업 배열이
...
다항식 $G_5$의 계수는 55308 이하지만 $G_6$의 계수는 훨씬 커서, $28\le n\le49$에서는
...
@ 조건 $h\ge6$이고 $n\ge20$일 때, 쓸 만한 크기의 그래프는 $n\ge2^h-7$에서만 나온다.
항 $z^{-(2^h-1-n)}$의 계수를 본다.
...
@ 조건 $\sigma_j>y_j$이면 $x_j$는 1로, $l_j=1$이거나 $y_j$의 1비트 수가 $\sigma_j$와
...
그래프를 돌려준다. 인자 |self|가 참이면 원래 그래프에 없던 자기 고리를 정점마다
새로 달고, 거짓이면 자기 고리를 하나도 만들지 않는다. 인자 |directed|가 참이면 원래
|u|에서 |v|로 가는 호가 없던 자리에 호를 놓고, 거짓이면 새 그래프가 무향이 되어
원래 |u|와 |v| 사이에 간선이 없던 자리에 간선을 놓는다. 뒤의 경우 원래 그래프도
무향이어야 한다(곧 호가 짝을 지어 있어야 한다).

인자 |cp|가 참이면 실제로는 여집합을 두 번 취한다. 그러면 새 그래프는 사실상 옛
그래프의 복사본이 되는데, 다만 중복된 호(와 경우에 따라 자기 고리)는 사라진다.
인자 |cp|가 무엇이든 유틸리티 필드에 담겨 있던 정보는 복사되지 않으며, 호의 길이는
모두 1로 놓인다.

함수 |Complement|가 돌려주는 그래프에는 쓸모 있을지 모를 성질이 하나 있다. 정점 |v|에
...
드는) 정점마다 호나 간선을 낸다. 필드 |v->arcs->tip|들이 엄격히 내림차순이 되는
성질이 여기서 나온다.
...
방법은 호들의 집합을 합치거나 교차시키는 것이다. 함수 |Gunion(g,gg,multi,directed)|은
...
정점을 같은 것으로 본다는 뜻이다. 그래프 |gg|의 정점 수가 |g|보다 많으면, 남는 정점과
...
@ 인자 |multi|가 참이면 새 그래프에 중복 간선이 생길 수 있다. 그래프 |g|에 |u|에서 |v|로
가는 호가 $k_1$개, |gg|에 $k_2$개 있다고 하자. 인자 |multi|가 참이면 합집합에
...
교집합 그래프에서 호의 길이는 조금 더 까다롭다. 그래프 |g|에 중복 호가 있으면 먼저 그
최소 길이 |l|을 셈한다. 그런 다음 |l|과 |gg|의 대응하는 호들의 길이 사이의
최댓값을 취한다. 인자 |multi|가 거짓이면 그 최댓값들 가운데 최솟값만 살아남는다.

@ 함수 |Gunion|은 |Complement|처럼 |tmp|(|U.V|) 요령으로 |u|에서 이미 기록한 호를
...
@ 각 정점마다 |g|의 호와 |gg|의 호를 차례로 훑어 |vv|에서 잇는다. 그래프 |gg|가
...
@ 필드 |tmp|가 |u->tmp==vv|이면 |u|로 가는 호를 이미 봤다는 뜻이고, 그때 |tlen|은
...
@ 함수 |Intersection|은 넉 개의 임시 필드를 쓴다: |tmp|(|U.V|), |tlen|(|Z.A|),
...
@ 값 |l|은 |g|쪽 최소 길이와 |gg|쪽 길이의 최댓값이다. 인자 |multi|가 아니면 여러
최댓값 중 최솟값만 살린다.
...
인자 |directed|가 거짓이면 선그래프는 |g|의 간선마다 정점 하나를 갖고, 두 정점은 그에
대응하는 두 간선이 공통 정점을 가질 때만 이웃이다.

인자 |directed|가 참이면 선그래프는 |g|의 호마다 정점 하나를 갖고, |u|에 대응하는
...
@ 함수 |restore|는 |g|에 얹은 임시 자료(정점 |map|과 짝호 |Tip|)를 걷어내 |g|를
점령의 흔적 없이 되돌린다. 값 |cnt|개의 선그래프 정점까지만 훑으므로, 중간에
실패해도 부분 복원에 쓸 수 있다.
...
선그래프 정점 |u|를 만든다. 그래프 |g|가 무향 관례를 어기면 |restore| 후
...
그래프 $g$에 호가 $m$개, 정점이 $n$개 있고 $g'$에 호가 $m'$개, 정점이 $n'$개 있다고
하자. 두 그래프 $g$와 $g'$의 {\sl 데카르트 곱\/}에는 호가 $mn'+m'n$개 있는데, $g$에 $u$에서
...
@ 함수 |Product(g,gg,type,directed)|는 이 세 가지 가운데 하나의 곱 그래프를 낳는다.
인자 |type|이 |Cartesian|(0)이면 데카르트 곱, |Direct|(1)이면 직접 곱,
|Strong|(2)이면 강한 곱이다.

데카르트 곱에서 호의 길이는 그 호가 본뜬 원래 호의 길이를 그대로 복사한 것이고,
직접 곱에서 호의 길이는 그 호를 낳은 두 호 길이의 최솟값이다. 인자 |directed|가
...
부호가 0이면 |v|를 없애고, 1이면 남긴다. 조건 |k>1|이면 |v|를 서로 이웃이 아니면서
|v|가 갖던 이웃을 그대로 갖는 정점 $k$개로 쪼갠다. 조건 |k<0|이면 같은 |k|값을 가진
다른 모든 정점과 하나로 동일시한다.

@ 보기를 하나 들자. 그래프 |g|가 정점 $\{0,1,\ldots,9\}$의 회로여서 $k=(j\pm1)\bmod10$일
...
이웃한 정점은 $-1$(전의 5), 7, $7'$, $7''$이다. 정점 $-1$에 이웃한 정점은 전에 1이나
5나 9에 이웃했던 것들, 곧 $-2$(전의 2), 4, 6, 8이다. 정점 $-2$에 이웃한 정점은 전에
2나 3에 이웃했던 것들, 곧 $-1$(전의 1), $-2$(전의 3), $-2$(전의 2), 4다.
인자 |multi|가 거짓이면 중복 간선이 버려지고, |self|가 거짓이면 자기 고리가 버려진다.
...
인자 |description|은 유도 그래프의 이름 일부가 될 문자열이다. 빈 문자열을 주면
그 부분이 비는데, 그럴 때는 사용자가 유도 부호를 어떻게 매겼는지 드러내는 알맞은
이름을 유도 그래프의 |ID|에 손수 붙여 주기를 권한다.

인자 |directed|가 거짓이면 입력 그래프가 무향이라고 보고 출력 그래프도 무향으로
만든다. 인자 |multi|가 거짓일 때, 여러 호를 대표하는 호의 길이는 그 여러 호 길이의
최솟값이 된다.

@ 함수 |Induced|에는 위에서 말하지 않은 특별한 기능이 하나 더 있다. 어떤 정점 |v|의
...
@ 유도 그래프를 짓는 핵심은 |g|의 호를 |ng|의 호로 옮기는 대목이다. 정점 |v|가
|k|개로 쪼개졌으면 그 복제본들 사이에 호를 낳는다. 인자 |multi|가 아니면 |tmp|와
...
끝난다. 그래프 |g|의 정점 |v| 자리에 그래프 |v.subst|의 복사본을 넣는다.
...
옮긴다. 인자 |multi|가 아니면 |tmp|·|tlen| 요령으로 중복을 거른다.
...
@ 그래프 |g|의 |map|과 |ng|의 임시 필드를 말끔히 치운다. \CEE/는 공용체라 |ind|를
...
@ 함수 |Induced|의 손쉬운 응용 둘이다. 첫째는 간단한 보기랄 만하다. 크기가 |n1|과
...
@ 함수 |Wheel|은 |indGraph| 요령으로, 한 정점을 순환(또는 회로)으로 치환한다.
...
@ 상수 |indGraph|는 유도 부호가 이 값 이상이면 |subst| 필드를 본다는 뜻의 큰 상수다.

@<상수 정의@>=
const indGraph = 1000000000 // 유도 부호가 10억 이상이면 |subst|를 본다

@* 시험. 알려진 조합론적 사실로 생성기를 검증한다. 함수 |Board|는 완전 그래프와
퀸 그래프로 확인한다.
...
@ 호출 |Complete(n)|은 $n$개 정점 각각이 나머지 $n-1$개와 이어진 완전 그래프다.
크기 $3\times4$ 판의 룩(|piece=-1|)과 비숍(|piece=-2|) 그래프를 합치면 퀸 그래프가
된다.
...
@ 크기 $3\times4$ 판에서 각 칸의 룩 이동은 $(3-1)+(4-1)=5$개다.
...
@ 함수 |Simplex|의 삼각형 배열($n=3,d=2$)은 정점 10개, |Petersen|은 정점 10개에
...
@ 함수 |Petersen|은 정점 10개 모두 차수 3에, 길이 5 미만의 회로가 없다.
...
@ 변환기들도 알려진 사실로 검증한다. 완전 그래프 $K_5$의 여집합은 간선이 없고, 빈 그래프의
여집합은 완전 그래프다.
...
@ 완전 그래프 $K_5$와 $K_5$의 교집합은 다시 $K_5$이다.
...
@ 완전 그래프 $K_2$의 데카르트 곱은 4-순환($C_4$)이다.
