...
func SaveGraph(g *gbgraph.Graph, filename string) (int64, error) {
	if g == nil || g.Vertices == nil {
		return 0, gbgraph.ErrMissingOperand
	}
...
func RestoreGraph(filename string) (*gbgraph.Graph, error) {
	...
	for i := int64(0); i < fileM; i++ {
		a := &arcs[i]
		var vtip *gbgraph.Vertex
		if vtip, ok = readVertexPtr(r, vertices); !ok {
			r.RawClose()
			return nil, gbgraph.ErrSyntaxError
		}
		a.Tip = vtip
		var anext *gbgraph.Arc
...
func readVertexPtr(r *gbio.Reader, vertices []gbgraph.Vertex) (*gbgraph.Vertex, bool) {
	c := r.GbChar()
	switch c {
	case 'V':
...
func readArcPtr(r *gbio.Reader, arcs []gbgraph.Arc) (*gbgraph.Arc, bool) {
	c := r.GbChar()
	switch c {
	case 'A':
		idx := int64(r.GbNumber(10))
...
func readCommaArcPtr(r *gbio.Reader, arcs []gbgraph.Arc) (*gbgraph.Arc, bool) {
	if r.GbChar() != ',' {
		return nil, false
	}
...
func readCommaField(r *gbio.Reader, typ byte, vertices []gbgraph.Vertex, arcs []gbgraph.Arc) (any, error) {
	if r.GbChar() != ',' {
		return nil, gbgraph.ErrSyntaxError
	}
	switch typ {
...
func appendVertexPtr(b *strings.Builder, v *gbgraph.Vertex, vidx map[*gbgraph.Vertex]int64, anomalies *int64) {
	if v == nil {
		b.WriteByte('0')
		return
	}
...
func appendArcPtr(b *strings.Builder, a *gbgraph.Arc, aidx map[*gbgraph.Arc]int64, anomalies *int64) {
	if a == nil {
		b.WriteByte('0')
		return
	}
...
func appendField(b *strings.Builder, val any, typ byte, vidx map[*gbgraph.Vertex]int64, aidx map[*gbgraph.Arc]int64, anomalies *int64) {
	switch typ {
	case 'I':
		v, _ := val.(int64)
		fmt.Fprintf(b, "%d", v)
...
func graphUtil(g *gbgraph.Graph, i int) any {
	switch i {
	case 0:
		return g.UU
	case 1:
...
func setGraphUtil(g *gbgraph.Graph, i int, val any) {
	switch i {
	case 0:
		g.UU = val
	case 1:
...
func vertexUtil(v *gbgraph.Vertex, j int) any {
	switch j {
	case 0:
		return v.U
	case 1:
...
func setVertexUtil(v *gbgraph.Vertex, j int, val any) {
	switch j {
	case 0:
		v.U = val
	case 1:
...
func arcUtil(a *gbgraph.Arc, i int) any {
	if i == 0 {
		return a.A
	}
	return a.B
...
func setArcUtil(a *gbgraph.Arc, i int, val any) {
	if i == 0 {
		a.A = val
	} else {
		a.B = val
