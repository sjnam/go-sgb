...
(\.\\나 \."나 줄바꿈은 안 된다), 길이는 4095자 이하라야 한다. 필드 |g.ID|는
...
넷째 필드가 그렇다). 함수 |RestoreGraph|는 |Vertex| 포인터가 1보다 큰 상수 값을
갖는 것은 허용하지 않으며, |Arc| 포인터가 와야 할 자리에 \.1이 오는 것도
허용하지 않는다.

타입 |Vertex|와 |Arc| 명세는 파일 첫머리의 유틸리티 타입 뒤에 적힌 수와 정확히
같은 개수라야 한다. 마지막 |Arc| 다음에는 특별한 검사합 줄이 와야 하는데,
앞선 모든 줄의 자료와 아귀가 맞는 수이거나, 아니면 음수라야 한다(음수는
검사하지 않는다). 검사합 줄 뒤의 정보는 모두 무시된다.

함수 |SaveGraph|가 만든 파일을 손으로 고치지 않는 것이 좋다. 검사합이 어긋나면
모든 것이 헛수고가 되기 쉽다. 다만 파일 맨 앞에는 `\.*'로 시작하는 줄을
주석으로 더 넣어도 된다. 그런 줄은 검사합의 대상이 되지 않는다.

@ 프로그램의 뼈대다. 패키지 |gbio|의 입출력 관례로 파일을 읽고, 표준 라이브러리로
쓴다. 문자열 |util_types|의 각 자리가 어느 유틸리티 필드를 가리키는지 짚어 주는 세
...
@ 문자열 |util_types|의 자리 0--5는 정점의 |U|--|Z|, 6--7은 호의 |A|--|B|, 8--13은
...
@* 그래프 되살리기. 호출 |RestoreGraph("foo.gb")|는 파일 |"foo.gb"|에 정의된 그래프를
...
@ 함수 |field|는 한 필드를 그 타입 부호에 따라 채운다. 첫 필드가 아니면 앞에 쉼표를
...
@ 문자열은 여는 따옴표로 시작한다. 함수 |readString|이 줄을 이어 붙여 닫는 따옴표까지
읽는다.
...
@ 함수 |readString|은 여는 따옴표를 이미 읽었다고 보고, 닫는 따옴표까지의 내용을
...
@ 마지막으로 짝 호를 잇는다. 함수 |save_graph|가 짝을 이웃하게 뽑아 두므로, 되살린
...
호출 |SaveGraph(g, "foo.gb")|는 파일 \.{foo.gb}를 만드는데, |g|가 앞서 말한
...
@ 함수 |moveItem|은 항목을 현재 줄에 이어 붙이되, 79자를 넘으면 새 줄로 넘긴다.
...
@ 함수 |SaveGraph|는 먼저 호마다 번호를 매긴다. 정점을 차례로 훑으며 아직 번호 없는
...
구조(이웃과 거리)가 모두 같아야 한다. 메서드 |Partner|도 제대로 이어져야 한다.
