...
같은 검사를 수행한다.
@c
// 패키지 |gbio|는 GraphBase 데이터 파일을 읽는 입출력 모듈이다.
package gbio
...
@<입출력 오류@>=
// |IOErrors|는 입출력 루틴이 알아챈 이상들의 비트 집합이다.
type IOErrors int64
...
@<파일 상태@>=
// |File|은 읽는 중인 GraphBase 데이터 파일 하나다.
type File struct {
...
원본은 파일이 안 열리면 컴파일 때 정한 |DATA_DIRECTORY|를 접두어로 붙여
