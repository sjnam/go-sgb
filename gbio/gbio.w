...
원본에는 ``이 루틴들을 다른 시스템으로 옮길 때는 손볼 곳이 있을 수 있으며, 그럴
만한 대목은 찾아보기의 `system dependencies' 항목에 모아 두었다''는 당부가
붙어 있었다. 문자 부호와 파일 이름 규약이 그런 대목이었는데, \GO/에서는 문자열이
어디서나 같은 바이트열이고 경로도 표준 라이브러리가 갈무리하므로 그 목록은
거의 비게 되었다.

원본의 상태는 모조리 \CEE/ 전역 변수였다---한 번에 파일 하나만 읽을 수 있다는
뜻이다. 여기서는 그 일습을 |File| 구조체에 거두어들였으므로, 여러 파일을
동시에 읽어도 서로 방해하지 않는다. 설치한 사람이 이 모듈이 제대로 도는지
가려낼 수 있도록 원본이 함께 뽑아내던 검증 프로그램 \.{test\_io.c}는 \GO/ 시험
파일 \.{gbio\_test.go}가 되어 마지막 절에서 같은 검사를 수행한다.
@c
package gbio
...
오류가 없다고 받아들인다. 소수 $p$는 검사합 갱신식 $2a+c$가 32비트로도 넘치지
...
바로 $k$다. 표 |imap|에는 96개 문자---눈에 보이는 표준 ASCII 문자 94개에
공백과 개행---가 들어 있다. (EBCDIC을 쓴다면 왼쪽 홑따옴표 자리에 센트
기호를, 물결표 자리에 또 다른 글자를 놓으라던 시절의 당부가 원본에 남아
있는데, 그 시절을 추억하는 뜻에서 적어 둔다.)

표 |imap|에 없는 문자들은 모두 같은 내부 부호 |unexpectedChar|를 받는다.
그런 문자는 GraphBase 파일에서 가급적 피해야 한다. 표 |icode|는 이중의
...
@ 함수 |NextLine|은 데이터의 다음 줄을 버퍼로 읽어들이고 마법의 수를 그에 맞게
...
함수 |Char|를 부르면 줄 안의 현재 위치 |pos|가 전진한다---이미 줄 끝에
...
처음 나타나기 직전까지의 문자열을 준다. 문자 |c|가 |'\n'|이면 줄 끝까지다.
문자 |c|가 나타나지 않으면 줄 끝에 늘 놓여 있는 개행이 문자열의 마지막 문자가
...
@ 호출 |Open("foo")|은 |RawOpen|의 좀 더 억센 판본으로, \.{words.dat} 같은
...
담은 검사합 매개변수 줄인가---이 넷이다. 두 값 $l$과 $m$은 |totLines|와
|finalMagic|에 갈무리되었다가 파일 끝에서 대조된다.

머리글 넉 줄은 |fillBuf|로 직접 읽는다. 함수 |NextLine|을 거치지 않으므로 줄
...
올바른지 확인한다. 함수 |Close|는 |Open|과 마찬가지로 문제가 하나라도 눈에
띄었으면 그 |IOErrors|를, 아니면 |nil|을 돌려준다.

마지막 줄은 |"* End of file "|에 이어 따옴표에 싸인 파일 이름이라야 한다.
버퍼를 비우고 |moreData|를 거짓으로 만드는 것으로 이 파일에 대한 입출력
루틴들은 사실상 멈춘다.
...
@ 앞에서 |Open|이 머리글 넉 줄을 어떻게 검증하는지 적었으니, 그 규약이 정말
지켜지는지 확인한다. \.{test.dat}를 한 줄씩 망가뜨려 임시 파일로 만들고,
줄마다 제 몫의 오류 비트가 서는지 본다. 덤으로 없는 파일과 검사합이 어긋난
파일도 시험한다.
...
func TestOpenChecksHeader(t *testing.T) {
	...
	for _, c := range cases {
		@<머리글 |c.line|을 망가뜨려 |c.want|가 서는지 본다@>
	}
	@<없는 파일과 검사합이 어긋난 파일을 시험한다@>
}

@ 파일 이름은 첫째 줄 검사에 쓰이므로 반드시 \.{test.dat}로 둔다. 망가뜨릴
줄은 통째로 |"x"|로 갈아 치운다.
...
@ 마지막 줄을 망가뜨리면 |Close|가 |BadLastLine|을 알아채야 하고, 데이터
줄을 고치면 검사합이 어긋나야 한다.
...
@ 줄을 나누고 다시 잇는 잔심부름이다. 파일은 늘 개행으로 끝나므로 마지막
빈 조각은 버린다.
