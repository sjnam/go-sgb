...
type Index struct {
	htab [5][hashPrime]*gbgraph.Vertex
}

// Weight returns the weighted frequency stored in vertex v's U field.
func Weight(v *gbgraph.Vertex) int64 { i, _ := v.U.(int64); return i }

// Loc returns the letter-position index (0–4) stored in arc a's A field.
func Loc(a *gbgraph.Arc) int64 { i, _ := a.A.(int64); return i }
...
func Words(n int64, wtVec []int64, wtThreshold, seed int64) (*gbgraph.Graph, *Index, error) {
	rng := gbflip.New(seed)
	...
	if usingDefault {
		...
		if flacc >= float64(0x60000000) {
			return nil, nil, gbgraph.ErrVeryBadSpecs
		}
	...
	ix := &Index{}
	if n > 0 && stackPtr != nil {
		sorted := gbsort.LinksSort(stackPtr, rng)
		remain := n
		curIdx := int64(0)
...
func (ix *Index) insertWord(g *gbgraph.Graph, curVertex *gbgraph.Vertex, word [5]byte, weight int64) {
...
func (ix *Index) FindWord(q string, f func(*gbgraph.Vertex)) *gbgraph.Vertex {
	if len(q) < 5 {
		return nil
	}
	var word [5]byte
