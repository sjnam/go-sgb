...
|wtVector|가 가리키는 표에 따라 계산한다(잠시 뒤에 규칙을 설명한다). 인자 |wtVector|가
...
인자 |wtVector|는 아홉 정수 $(a,b,w_1,\ldots,w_7)$의 배열을 가리킨다. 각 낱말의
...
@ 보기를 들어 보자. 호출 |Words(2000,nil,0,0,dir)|을 부르면 기본 무게를 써서 영어에서
...
함수 |Words|는 씨앗으로 난수 스트림을 하나 열고, 무게 벡터가
...
변수 |usedDefault|는 나중에 표식 문자열을 지을 때 쓰려고, |wtVector|를 기본값으로
...
@* 무게를 검증한다. 함수 |Words|가 맨 먼저 할 일은 견주어 보면 사소하다. 우리는 조건
$$\max\bigl(\vert a\vert, \vert b\vert\bigr)
...
@ 함수 |readWords|는 파일을 한 줄씩 훑으며 자격을 갖춘 낱말을 스택에 쌓는다. 조기
...
@ 함수 |Number|는 현재 자리에 숫자가 없으면 오류 없이 0을 준다. 그 덕분에
...
함수 |blanked|는 거기서 |k|번째 글자의 몫을 빼, 그 자리를 지운 네 글자 무늬의
...
@ 함수 |matchExcept|는 두 낱말이 |k|번째 자리를 뺀 네 자리에서 모두 같은지 본다.
...
@ 함수 |makeWordHash|는 다섯 표를 각각 |hashPrime| 칸으로 마련한다. 함수 |insert|는
...
@* 낱말 찾기. 함수 |Words|가 그래프 |g|를 만든 뒤에는, 또 다른 함수로 주어진 낱말과
꼭 맞거나 거의 맞는 정점을 찾을 수 있다.

함수 |FindWord(g, q, f)|는 다섯 글자 낱말 |q|와 꼭 맞는 정점이 그래프에 있으면 그것을
...
남는다. 무게 |{1,1}|은 $a=b=1$이라 흔하거나 고급인 4494개다. 기본 무게에 문턱 0이면
...
@ 함수 |FindWord|로 그래프의 첫 정점(무게가 가장 큰 낱말)을 그 이름으로 되찾아
...
@ 너무 큰 무게 벡터는 걸러져야 한다. 무게 $w_1=100000$이면 $C_1\cdot w_1$이 $2^{30}$을
