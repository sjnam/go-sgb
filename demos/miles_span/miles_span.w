...
성기게 또는 더 빽빽하게 할 수도 있다. 인자 |northWeight|·|westWeight|·|popWeight|에
...
함수 |Miles|는 무게가 큰 |n|개 도시를 뽑는데, 무게가 $0$이 아니면 도시들의 무게가
꼭 같은 일이 드물어 난수로 동점을 가릴 일이 거의 없기 때문이다.

특별한 옵션 \.{-g}\<파일명>은 다른 모든 옵션을 제친다. 함수 |SaveGraph|로 예전에
...
@ 타입 |solver|는 그래프와 mem 계수기, 그리고 진단 출력 설정을 한데 묶는다. \CEE/
...
@ 인자 |verbose|일 때, 여러 알고리즘이 찾은 간선을 |report|가 알린다.
...
증명은 이렇다. 트리 $T$를 $F$는 담되 $e$는 담지 않는 최소 신장 트리라 하자. 트리 $T$에
$e$를 더하면 회로가 하나 생기는데, 그 회로에는 $e$가 아닌 간선 $e'$이 있어
$F$ 안의 정점에서 $F$ 밖의 정점으로 간다. 그래프 $T\cup e$에서 $e'$을 빼면 총길이가
$T$보다 크지 않은 신장 트리 $T'$이 나온다. 그러므로 $T'$은 $F\cup e$를 담는
최소 신장 트리다. 증명 끝.

@ 함수 |Miles|가 짓는 그래프에는 특별한 성질이 있고, 그것을 이용할 수 있다면 이용해도
반칙이 아니다.
...
없애는 데 도움이 된다. 변수 |o|와 |oo|가 놓인 자리가 프로그램 본문을 어지럽히지
...
조건 |a->tip>v|는 |tip|이 |v|보다 나중 정점일 때 참으로, 간선을 한 번만(작은 쪽
끝점에서) 보게 한다.
...
@ 함수 |krusk|가 할 나머지 일은 ``동치 알고리즘'' 또는 ``합집합/찾기(union/find)''
...
@ 함수 |initQueue|를 부르는 데는 mem을 물리지 않았음을 눈여겨보라---그 안에서 세는
...
@ 정점 |t|의 이웃을 훑는다. 이미 본(그러나 아직 모르는) 이웃은 더 짧은 길을 찾았을
...
@* 이진 힙. 함수 |jarPr|를 마치려면 네 우선순위 큐 함수를 채워야 한다. Jarn\'\i k은
...
현실적인 자료에서 정말 $m\log n$처럼 움직일지도 분명치 않다. 복잡도 $O(m\log n)$은
...
나타낸다. 마디마다 포인터 넷을 둔다. 필드 |parent|는 마디의 부모(뿌리면 |nil|),
...
@ 필드 |tag|가 무엇을 위한 것인지 밝혀 두자. 어떤 마디의 자식들의 계수가
...
필드 |fHeap.dist|는 레지스터에 있다고 보아 읽는 데 mem을 물리지 않는다.
...
예전 부모 |p|의 계수는 1 줄어든다. 마디 |p|가 뿌리이면 그것으로 끝이다. 그렇지 않고
...
마디 |p|가 뿌리이면 계수만 1 줄이고 끝낸다. 뿌리가 아닌데 표식이 없었으면 표식을
...
힙 |fHeap|이 우리가 지울 마디 $v$를 가리킨다는 것은 안다. 좋은 일이지만, |fHeap|의
...
@ 먼저 지울 마디의 자식들을 뿌리들의 고리에 풀어 놓는다. 힙 |fHeap|의 |rank_tag|가
2보다 작으면 계수가 $0$이라 자식이 없으니 오른쪽 형제부터 훑기 시작하면 된다.
자식이 있으면 자식들의 고리를 뿌리 고리에 이어 붙이고, 그 자식들의 |parent|를
모두 |nil|로 만든다---이제 그들이 뿌리이기 때문이다. 마디 |v|는 훑기를 시작할 자리를
가리키게 된다.
...
@ 마디 |v|의 계수 |r|이 아직 |new_roots|에 없으면 그 자리에 놓는다. 이미 그 계수의
...
있는 원소의 수가 이항계수 $k\choose l$이라는 것이다. 원소 $k$개짜리 집합의 부분집합을
훑는 되추적 트리도 같은 구조다. 조건 $k=5$인 이항 큐 트리를 Jill~C. Knuth가 그린
...
@ 우리가 이항 큐로 하려는 연산 대부분이 다음의 기본 서브루틴에 기댄다. 함수 |qunite|는
...
이 절차는 이진 덧셈의 자리올림과 똑같이 움직인다. 값 |m|과 |mm|을 이진수로 보면
...
|qenque|로, 새 호를 분할 상환 $O(1)$ 시간에 큐에 넣는다. 함수 |qmerge|는 한 이항 큐를
다른 이항 큐에 합치는데, 분할 상환 실행 시간이 작은 쪽 큐의 마디 수의 로그에
비례한다. 함수 |qdelMin|은 키가 가장 작은 마디를 빼는데, 분할 상환 실행 시간이 큐
크기의 로그에 비례한다.
...
@ 키 최소 마디 빼기다. 식 |m&(m-1)|이 |m|에서 최하위 1비트를 지운 값이라는 잘
알려진 요령을 쓴다.
...
@ 조각을 나타내는 데는 앞서 정의한 유틸리티 필드 여럿을 다시 쓴다. 필드 |lsib|(|V.V|)과
|rsib|(|W.V|) 포인터는 작은 리스트의 조각들 사이에 쓰이며 이 리스트는 두 겹으로
이어진다. 포인터 |sm|이 첫 작은 조각을, |sm.rsib|이 그다음을 \dots\ 가리키고, |tl.lsib|이
끝에서 둘째를, |tl|이 마지막을 가리킨다. 필드 |sm.lsib|과 |tl.rsib|은 뜻이 없다.
리스트 |largeList|는 |rsib| 포인터로 한 겹으로 이어지며 |nil|로 끝난다.
...
@ \lsqrtn\ 과 \usqrtn\ 을 셈하는 뻔하지 않은 방법이 있다. 값 $\sqrt n$은 작고 산술은
...
@ 조각 |u|가 여전히 작으면 작은 리스트의 맨 끝으로 옮긴다.
...
@ 조각 |u|가 방금 커졌으면 작은 리스트에서 떼어 큰 리스트 앞에 붙인다.
...
행렬을 놓을 자리가 있어야 한다. 함수 |cherTarKar|가 여태 |z| 유틸리티 필드를 쓴 적이
없으므로, 정점 레코드의 그 필드에 임의 접근해 쓰기로 한다. 유틸리티 필드 |v|도
...
|g.Vertices[j*loSqrt+k].Z.I|에, 그에 대응하는 호는 |.V.A|에 둔다. 상수 |INF=30000|은
모든 간선 길이의 상한이다.
...
@ 필드 |comp|이 |nil|이 아닌 정점마다, 그 조각 대표의 |findex|를 따라 자기 |findex|를
...
@ 함수 |note_edge|는 |qtraverse|가 훑는 이항 큐의 간선마다 불린다. 변수 |kk|는 이 호가
나가는 조각의 번호다.
...
프로시저 |cherTarKar|는 근처에도 못 간다. 그것이 밟는 걸음 하나하나가 그럴듯하고
...
프로시저 |krusk|는 날씬하게 다듬은 합집합/찾기 알고리즘을 주면 더 빨라질지도
