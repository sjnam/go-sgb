% go-sgb: Knuth의 Stanford GraphBase를 한글 GWEB로 옮긴 것.
% 이 파일은 football.w를 Go로 이식한다.
@i ../../gbtypes.w

\input kotexgweb
\def\title{FOOTBALL}
\def\<#1>{\hbox{$\langle$\rm#1$\rangle$}}

@* 들어가며. 이 시연 프로그램은 {\sc GB\_GAMES}가 지은 그래프로 \.{football}이라는
대화형 프로그램을 만든다. 이 프로그램은 한 팀이 다른 팀보다 터무니없이 큰 차로
앞선다고 ``증명''하는, 어처구니없이 긴 점수 사슬을 찾는다.

프로그램은 시작 팀을 묻는다. 그냥 \<return>을 치면 끝나고, 그렇지 않으면 팀 이름
(이를테면 `\.{Stanford}')을 친 뒤 \<return>을 쳐야 한다. 그다음 또 다른 팀을
묻는다. 여기서 그냥 \<return>을 치면 돌아가 새 시작 팀을 묻고, 그렇지 않으면 또
다른 이름(이를테면 `\.{Harvard}')을 대야 한다. 팀 이름은 \.{games.dat}에 적힌
그대로여야 한다(예: |"Berkeley"|가 아니라 |"California"|).

@ 그러면 프로그램은 시작 팀에서 다른 팀으로 가는 사슬을 찾아 보여 준다. 이를테면
이런 것이 나온다.
$$\vbox{\halign{\tt#\hfil\cr
 Oct 06: Stanford Cardinal 36, Notre Dame Fighting Irish 31 (+5)\cr
 Oct 20: Notre Dame Fighting Irish 29, Miami Hurricanes 20 (+14)\cr
 Jan 01: Miami Hurricanes 46, Texas Longhorns 3 (+57)\cr
 Nov 03: Texas Longhorns 41, Texas Tech Red Raiders 22 (+76)\cr
 Nov 17: Texas Tech Red Raiders 62, Southern Methodist Mustangs 7 (+131)\cr
 Sep 08: Southern Methodist Mustangs 44, Vanderbilt Commodores 7 (+168)\cr
\omit\qquad\vdots\cr
 Nov 10: Cornell Big Red 41, Columbia Lions 0 (+2188)\cr
 Sep 15: Columbia Lions 6, Harvard Crimson 9 (+2185)\cr}}$$
사슬 위 각 경기는 앞 팀이 뒤 팀을 이긴 점수 차를 |del|로 삼아 이어지고, 괄호 안은
사슬을 따라 쌓인 점수 차의 합이다.

이 사슬이 최적이라는 보장은 없다. 그저 이 프로그램이 어림한 최선일 뿐이다. 사실
스탠퍼드에서 앞으로 찾는 대신 하버드에서 거꾸로 찾도록 이 프로그램을 고치면
$+2279$점이라는 더 큰 차를 세우는 사슬이 나온다. 이보다 더 나은 사슬을 찾는
알고리즘을 궁리하는 것은 재미있는 일일 것이다.

@ 사실 이 프로그램에는 두 갈래가 있다. 그냥 \.{football}이라고 부르면 단순한
``탐욕 알고리즘''이 찾은 사슬을 얻는다. 그러나 \UNIX/ 명령줄 관례에 따라
\.{football} \<수>라고 부르면 프로그램이 더 열심히 일한다. \<수>가 클수록 셈은
늘지만 대개 더 나은 사슬을 낸다.

이를테면 단순한 탐욕 알고리즘은 스탠퍼드가 하버드보다 겨우 $781$점 앞선다고 하고,
\.{football}~\.{10}은 이를 $1895$로 올린다. 위의 보기는 \.{football}~\.{4000}에
해당한다.

비밀 옵션 \.{-v}는 계층 탐색의 진행을 보이고, \.{-D}\<디렉터리>는 자료 파일이
놓인 곳을 바꾼다(기본값 \.{data}).

@ 프로그램의 뼈대다. \CEE/ 원본은 전역 |g|·|width|·|next_node| 따위에 기대지만,
우리는 그래프 상태를 |chainer| 구조체에 담아 패키지 수준 가변 상태를 피한다.

@c
package main

import (
	"bufio"
	"fmt"
	"io"
	"log"
	"os"
	"strconv"
	"strings"
	@#
	"github.com/sjnam/go-sgb/gbflip"
	"github.com/sjnam/go-sgb/gbgames"
	"github.com/sjnam/go-sgb/gbgraph"
)

type (
	Arc = gbgraph.Arc
	Graph = gbgraph.Graph
	Vertex = gbgraph.Vertex
)

@<자료 구조@>
@<탐욕 알고리즘@>
@<계층 탐욕 알고리즘@>
@<이중 성분 계산@>
@<터미널 상호작용@>

func main() {
	@<명령줄 옵션을 읽는다@>
	@<그래프 마련하기@>
	c := &chainer{g: g, width: width, verbose: verbose,
		rng: rng, out: os.Stdout}
	c.run()
}

@ 명령줄을 훑는다. 수 인자는 계층 너비, \.{-v}는 진행 표시, \.{-D}\<경로>는 자료
디렉터리다. 음수 너비는 절댓값을 쓴다(하이픈을 쓴 사용자를 위해).

@<명령줄 옵션을 읽는다@>=
var dir string
var width int64
verbose := false
for _, arg := range os.Args[1:] {
	switch {
	case arg == "-v":
		verbose = true
	case strings.HasPrefix(arg, "-D"):
		dir = arg[2:]
	default:
		w, err := strconv.ParseInt(arg, 10, 64)
		if err != nil {
			log.Fatalf("사용법: %s [탐색너비][-v][-D디렉터리]", os.Args[0])
		}
		if w < 0 {
			w = -w
		}
		width = w
	}
}

@ 필요한 것보다 좀 더 일반적인 자료 구조를 쓴다. 나중에 구현할 더 복잡한
어림짐작을 미리 내다본 것이다. 있을 수 있는 해 전체는 하나의 백트래킹 나무로 볼
수 있는데, 각 노드에서 뻗는 가지는 그 노드 다음에 올 수 있는 경기들이다. 우리는
그 거대한 나무의 아주 작은 일부만 살펴본다.

백트래킹 나무의 각 노드는 어느 팀에서 다음 팀으로 가는 한 경기를 담는다.
필드 |game|은 그 경기 호, |totLen|은 |start|에서 여기까지 쌓인 점수 차, |prev|는 지금
팀을 알려 준 노드다. 필드 |next|는 같은 계층의 노드를 잇는다(계층 탐욕에서 쓴다). \CEE/ 원본은 \.{-v}용
계층·순번 표를 |next| 포인터에 억지로 채워 넣지만, 우리는 |stratum|·|order|
필드를 따로 둔다.

@<자료 구조@>=
type node struct {
	game    *Arc
	totLen  int64
	prev    *node
	next    *node
	stratum int64 // \.{-v}: 이 노드가 놓인 계층
	order   int64 // \.{-v}: 그 계층 안 순번
}

@ 타입 |chainer|는 그래프와 탐색 상태를 한데 묶는다. 이중 성분 계산에 쓰는
|activeStack|·|settledStack|·|nn|·|dummy|와, 계층별 노드 리스트 |list|·|size|,
그리고 \.{-v} 진행 표시용 |mm|을 담는다.

@<자료 구조@>=
type chainer struct {
	g       *Graph
	width   int64
	verbose bool
	rng     *gbflip.RNG
	out     io.Writer

	activeStack  *Vertex // 활성 정점 스택의 꼭대기
	settledStack *Vertex // 찾은 이중 성분들의 스택
	nn           int64           // 지금까지 본 정점의 수
	dummy        *Vertex // |goal|의 가상 부모

	list []*node // 계층별 최선 노드 리스트
	size []int64 // 각 리스트의 노드 수
	mm   int64   // \.{-v}용 계수기
}

@* 그래프 마련. 호출 |Games(0,...)|로 1990년 시즌 전체 그래프를 짓는다. 함수 |Games|가 준
호에는 |u|가 낸 점수가 |Len|으로 실려 있다. 우리는 그 호에 |del| 필드를 더
얹는데, |del|은 |u|와 |v|의 점수 차다. 게임 그래프에서 |venue|였던 |A.I| 자리를
이제 |del|로 다시 쓴다.

난수 생성기는 우리가 열어 |GamesRNG|에 건넨다. \CEE/ 원본은 전역 스트림 하나를
쓰므로, 사용자가 모르는 팀 이름을 쳤을 때 아무 팀이나 하나 골라 보여 주는 대목이
|games|가 쓰다 만 스트림을 이어서 쓴다. 우리가 여기서 새 |RNG|를 열면 그 자리에서
원본과 다른 팀이 나온다---실제로 처음에 그런 어긋남이 있었다.

@<그래프 마련하기@>=
rng := gbflip.New(0)
g, err := gbgames.GamesRNG(0, 0, 0, 0, 0, 0, 0, 0, rng, dir)
if err != nil {
	log.Fatalf("그래프를 만들 수 없습니다: %v", err)
}
for v := range g.AllVertices() {
	for a := range v.AllArcs() {
		if g.Index(a.Tip) > g.Index(v) { // 짝마다 한 번만
			a.A.I = a.Len - a.Partner.Len // |del|
			a.Partner.A.I = -a.A.I
		}
	}
}

@* 탐욕. 이 프로그램의 으뜸 과제는 |del|을 경로 속 각 호의 길이로 삼아 |start|
에서 |goal|로 가는 가장 긴 단순 경로를 찾는 것이다. 이는 \\{NP}-완전 문제이고
가능한 경우의 수도 어마어마하다. 그래서 이 프로그램은 셈하기 어렵지 않은
어림짐작으로 만족한다. (연구자들에게 이로써 더 나은 어림짐작을 내놓아 보라고
도전장을 던진다. 모의 담금질은 좋은 결과를 낼까? 유전 알고리즘은 어떨까?)

아마 가장 먼저 떠오르는 방식은, 매 걸음에서 결국 |goal|에 이르는 것을 막지 않는
한 가장 큰 |del|을 고르는 단순한 ``탐욕'' 방식일 것이다. 그러니 그것부터
구현한다.

@ 정점 |v|에서 아무 이웃으로나 갈 수 있는 것은 아니다. 정점 |v| 자신이나 |start|에서
여기까지 오는 길에 이미 쓴 다른 정점을 건드리지 않고도 |goal|에 이를 수 있는
정점으로만 갈 수 있다.

게다가 |v|에서 지역적으로 가장 좋은 수가 |goal|로 곧장 가는 것이더라도, 그것이
마지막 기회가 아닌 한 그 수를 두고 싶지 않다. 사슬을 더 늘리는 편이 아마 더 나을
테니까. 그러지 않으면 이를테면 어떤 팀과 그 팀이 가장 크게 이긴 상대 사이의
사슬이 경기 하나로 끝나 버린다.

정점의 유틸리티 필드를 이렇게 쓴다: |blocked=U.I|(경로에 이미 쓴 정점),
|valid=V.V|(|goal|로 가는 정점의 표), |link=W.V|(표시 스택). \CEE/의 공용체와
달리 Go 필드는 서로 독립이라 게임 그래프의 필드와 겹치지 않는다.

@<탐욕 알고리즘@>=
func (c *chainer) greedy(start, goal *Vertex) *node {
	for v := range c.g.AllVertices() {
		v.U.I = 0   // |blocked|
		v.V.V = nil // |valid|
	}
	var curNode *node
	for v := start; v != goal; v = curNode.game.Tip {
		v.U.I = 1 // |blocked|
		curNode = newNode(curNode, 0)
		c.markReachable(v, goal)
		@<|v|에서 갈 수 있는 최선의 호를 골라 |curNode.game|으로 삼는다@>
	}
	return curNode
}

@ 정점 |v|의 호 가운데, 상대가 |goal|로 가는 표(|valid==v|)를 지녔고 |del|이 가장 큰
것을 고른다. 다만 |goal|로 바로 가는 호는 마지막 수단(|lastArc|)으로 미룬다 ---
그러지 않으면 사슬이 한 경기로 끝나 버린다.

@<|v|에서 갈 수 있는 최선의 호를 골라 |curNode.game|으로 삼는다@>=
d := int64(-10000)
var bestArc, lastArc *Arc
for a := range v.AllArcs() {
	if a.A.I > d && a.Tip.V.V == v {
		if a.Tip == goal {
			lastArc = a
		} else {
			bestArc = a
			d = a.A.I
		}
	}
}
if d == -10000 {
	curNode.game = lastArc // 마지막 수단
} else {
	curNode.game = bestArc
}
curNode.totLen += curNode.game.A.I // |del|

@ 함수 |newNode|는 낡은 노드 |x|를 |prev|로 삼는 새 노드를 만든다. Go의 GC 덕에 노드를
낱개로 할당해도 포인터가 그대로 유효하므로, \CEE/의 블록 할당은 필요 없다.

@<탐욕 알고리즘@>=
func newNode(x *node, d int64) *node {
	var tot int64
	if x != nil {
		tot = x.totLen
	}
	return &node{prev: x, totLen: tot + d}
}

@ 표준적인 표시 알고리즘으로 마지막 빠진 고리를 채운다: |goal|에서 거꾸로,
막히지 않은(|blocked==0|) 정점들을 훑어 |valid=v|로 표시한다. 정점 |goal|에서 그
정점까지 |v|를 건드리지 않고 닿을 수 있다는 뜻이다.

@<탐욕 알고리즘@>=
func (c *chainer) markReachable(v, goal *Vertex) {
	u := goal
	u.W.V = nil // |link|
	u.V.V = v   // |valid|
	for u != nil {
		a := u.Arcs
		u = u.W.V // 스택에서 꺼낸다
		@<|a|의 이웃 가운데 새로 닿는 것을 표시해 스택에 얹는다@>
	}
}

@ @<|a|의 이웃 가운데 새로 닿는 것을 표시해 스택에 얹는다@>=
for ; a != nil; a = a.Next {
	t := a.Tip
	if t.U.I == 0 && t.V.V != v { // 안 막혔고 아직 표 안 됨
		t.V.V = v
		t.W.V = u
		u = t // 스택에 얹는다
	}
}

@* 계층 탐욕. 더 나은 사슬을 얻는 한 방법은 다음 알고리즘인데, Pang Chen의 비슷한
@^Chen, Pang-Chieh@>
착상[Ph.D. thesis, Stanford University, 1989]에서 힌트를 얻었다.

(어쩌면 아주 거대한) 백트래킹 나무의 노드들이 함수 $h$에 의해 (꽤 적은 수의)
계층으로 나뉜다고 하자. 여기서 $h$는 $h(\hbox{자식})<h(\hbox{부모})$이고
$h(\hbox{goal})=0$이라는 성질을 가진다. 또 우리가 주어진 함수 $f(x)$를 최대로 하는
노드 $x$를 찾고 싶다고 하고, 게다가 {\sl 부모의 $f$ 값이 부모의 계층 안에서 비교적
클 때에만 자식의 $f$ 값도 자식의 계층 안에서 비교적 클 것\/}이라고 믿을 만하다고
하자. 그렇다면 각 계층에서 $f$ 값으로 매긴 위 $w$개 노드로만 백트래킹을 제한하는
것이 말이 된다.

@ 앞서 설명한 탐욕 알고리즘은 이 일반적인 접근의 특수한 경우로, $w=1$이고
$h(x)=-(x$로 이끄는 사슬의 길이$)$인 경우다. 이제 다듬을 알고리즘은 일반적인 $w$
값을 쓰고, 좀 더 뜻있는 계층 함수를 쓴다. 가장 긴 경로를 찾는 백트래킹 나무의
노드 $x$가 |start|에서 어떤 정점 $u=u(x)$까지의 경로에 대응한다고 할 때, $h(x)$를
|u|와 |goal| 사이에 놓이는 정점의 수로 삼는다. 여기서 ``사이에 놓인다''는 것은,
|start|에서 |u|까지의 단순 경로를 늘여 그 정점을 지나 |goal|까지 갈 수 있다는
뜻이다.

@ 계층 탐욕 알고리즘의 맨 위 얼개다. 계층마다, 곧 $h$의 값마다 노드의 연결
리스트를 하나씩 둔다. 필요한 노드의 수는 $w$ 곱하기 계층 수로 묶인다. 아직 다
살피지 않은 가장 높은 계층 |m|부터 하나씩 노드를 꺼내 그 자식들을 다시 알맞은
계층에 넣는다.

@<계층 탐욕 알고리즘@>=
func (c *chainer) stratified(start, goal *Vertex) *node {
	c.list = make([]*node, c.g.N)
	c.size = make([]int64, c.g.N)
	var curNode *node // |nil|은 나무의 뿌리
	m := c.g.N - 1    // 아직 다 살피지 않은 가장 높은 계층
	for {
		c.placeChildren(curNode, start, goal)
		for c.list[m] == nil {
			@<|m|을 낮추고 다른 리스트를 살필 채비를 한다@>
		}
		curNode = c.list[m]
		c.list[m] = curNode.next // 가장 높은 계층에서 노드 하나를 뺀다
		if c.verbose {
			@<|cur_node|의 진행 정보를 찍는다@>
		}
		if m == 0 { // 오직 |list[0]|에 노드 하나가 남는다
			break
		}
	}
	return curNode
}

@ 리스트는 |tot_len| 오름차순으로 두어, 머리(가장 작은 것)를 쉽게 뺀다. 조건 |h=0|일
때는 답이 하나뿐이므로 |width|개가 아니라 한 노드만 남긴다.

@<계층 탐욕 알고리즘@>=
func (c *chainer) placeNode(x *node, h int64) {
	@<리스트가 꽉 찼으면 가장 작은 노드를 밀어낸다@>
	@<노드 |x|를 |tot_len| 차례에 맞게 끼운다@>
}

@ 계층이 꽉 찼으면(|h>0|이며 |width|개이거나, |h==0|이며 이미 하나 있으면), |x|가
머리보다 작으면 버리고, 아니면 머리를 밀어낸다.

@<리스트가 꽉 찼으면 가장 작은 노드를 밀어낸다@>=
if (h > 0 && c.size[h] == c.width) || (h == 0 && c.size[0] > 0) {
	if x.totLen <= c.list[h].totLen {
		return // |x|를 버린다
	}
	c.list[h] = c.list[h].next // 노드 하나를 밀어낸다
} else {
	c.size[h]++
}

@ @<노드 |x|를 |tot_len| 차례에 맞게 끼운다@>=
var q *node
p := c.list[h]
for p != nil {
	if x.totLen <= p.totLen {
		break
	}
	q = p
	p = p.next
}
x.next = p
if q != nil {
	q.next = x
} else {
	c.list[h] = x
}

@ 큰 항목이 먼저 들어가도록 리스트를 뒤집는다.

@<|m|을 낮추고 다른 리스트를 살필 채비를 한다@>=
m--
var r *node
s := c.list[m]
for s != nil {
	t := s.next
	s.next = r
	r = s
	s = t
}
c.list[m] = r
c.mm = 0 // \.{-v} 찍기용 색인

@ \.{-v} 진행 정보다. 노드마다 계층 |m|과 그 안 순번 |mm|을 담은 표를 |next|
자리에 슬쩍 실어, 부모의 표까지 곁들여 찍는다.

@<|cur_node|의 진행 정보를 찍는다@>=
c.mm++
tip := "start"
if curNode.game != nil {
	tip = curNode.game.Tip.Name
}
pm, pmm := int64(0), int64(0)
if curNode.prev != nil {
	pm, pmm = curNode.prev.stratum, curNode.prev.order
}
curNode.stratum, curNode.order = m, c.mm
fmt.Fprintf(c.out, "[%d,%d]=[%d,%d]&%s (%+d)\n",
	m, c.mm, pm, pmm, tip, curNode.totLen)

@ 여담으로, 계층 알고리즘이 언제나 단순한 탐욕 알고리즘을 이길 것이라는 추측이
그럴듯해 보인다. 그러나 그 추측은 거짓이다. 이를테면 탐욕 알고리즘은 하버드가
스탠퍼드보다 $1529$점 앞선다고 매기는데, 계층 알고리즘은 |width=1|일 때 겨우
$1527$에 그친다.

다른 한편 탐욕 알고리즘은 곧잘 처참하게 실패한다. 아이비리그의 두 팀을 견줄 때
그것은 아이비리그와 패트리엇리그를 벗어나는 길을 찾아내지 못한다.

@* 이중 성분 계산. 함수 $h$를 셈하기는 얼마나 어려울까? 이어진 그래프 $G$와 두
두드러진 정점 $u$·$v$가 주어졌을 때, $u$에서 $v$로 가는 단순 경로에 나타날 수
있는 정점의 수를 세야 한다.

이는 $u$와 $v$ 양쪽에서 닿을 수 있는 정점의 수와 {\sl 같지 않다.\/} 이를테면
정점 $\{u,v,w,x\}$ 넷에 $x$에서 나머지 셋으로만 간선이 있는 ``발톱(claw)''
그래프를 생각해 보라. 이 그래프에서 $w$는 $u$에서도 $v$에서도 닿을 수 있지만,
그 둘 사이의 어떤 단순 경로 위에도 놓이지 않는다.

@ 이 문제를 푸는 가장 좋은 길은 아마 $G$의 이중 성분을, 적어도 그 일부를 셈하는
것이다. 다른 시연 프로그램 {\sc BOOK\_COMPONENTS}가 관련 이론을 꽤 자세히
설명하므로, 여기서는 그 알고리즘에 익숙하다고 보고 이야기한다.

그래프 $G$를 조금 키운 그래프 $G^+$를 생각하자. 정점 $v$에만 이웃한 가상 정점 $o$를 더한
것이다. 정점 $o$에서 시작하는 깊이 우선 탐색으로 $G^+$의 이중 성분을 구한다고 하자.
이 이중 성분들은 $o$와 $v$만 담은 이중 성분을 뿌리로 하는 트리를 이룬다. 그러면
$u$와 $v$ 사이의 경로에 놓이는 정점의 수는($v$ 자신은 빼고) $u$를 담은 이중
성분과, 그것에서 뿌리까지 사이에 있는 다른 이중 성분들의 정점 수다.

@ 엄밀히 말하면 분리점은 저마다 둘 이상의 이중 성분에 든다. 그러나 우리는 분리점을
트리에서 뿌리에 가장 가까운 쪽 이중 성분에 배정한다. 그러면 각 이중 성분의 정점이
곧 깊이 우선 절차가 한 묶음씩 뱉어 내는 정점들과 정확히 일치한다.

우리가 헤아리려는 이중 성분은 $B_1$, $B_2$, \dots, $B_k$인데, $B_1$은 $u$를 담은
이중 성분이고 $B_{j+1}$은 $B_j$에 딸린 분리점을 담은 이중 성분이며, $B_k$에 딸린
분리점이 $v$일 때 멈춘다. (흔히 $k=1$이다.)

주어진 그래프 $G$의 ``자식''은 정점 $u$를 없애고, $u$에 이웃했던 정점 $u'$에서
$v$로 가는 경로를 생각해 얻는다. 그러니 $u'$은 $B_1$ 안에 있거나 $B_1$에 딸린
분리점이다. 정점 $u$를 없애면 대개 $B_1$이 더 작은 이중 성분들의 트리로 쪼개지지만,
$B_2,\ldots,B_k$는 그대로다. 아래 구현은 이 관찰을 온전히 활용하지 않는데, 다시
셈하기를 피하려면 메모리가 아마 감당 못 할 만큼 들 것이기 때문이다.

@ 아래 프로그램은 {\sc BOOK\_COMPONENTS}에서 거의 그대로 베낀 것이다. 거기 있는
설명을 되풀이하는 대신 뜻있는 차이만 짚는다. 한 가지 차이는 깊이 우선 탐색을
정해진 자리, 곧 |goal|에서 시작한다는 점이다.

정점의 유틸리티 필드를 이렇게 쓴다: |rank=Z.I|(언제 처음 봤나), |parent=U.V|(누가
알려 줬나), |untagged=X.A|(첫 태그 안 된 호), |min=V.V|(성숙한 자손에서 얼마나
낮이 뛸 수 있나).

@<이중 성분 계산@>=
func (c *chainer) placeChildren(curNode *node, start, goal *Vertex) {
	@<모든 정점을 안 봄으로, 모든 호를 태그 안 됨으로 둔다@>
	c.bicomponentDFS(goal)
	@<자식마다 새 노드를 만들어 알맞은 계층에 넣는다@>
}

@ 정점의 |rank|를 무한대로 두는 것은 그 정점을 그래프에서 뺀 것과 같다. 노드 |cur_node|로
이끄는 걸음에 이미 쓴 정점과 |start|를 그렇게 빼 둔다.

@<모든 정점을 안 봄으로, 모든 호를 태그 안 됨으로 둔다@>=
for v := range c.g.AllVertices() {
	v.Z.I = 0      // |rank|
	v.X.A = v.Arcs // |untagged|
}
for x := curNode; x != nil; x = x.prev {
	x.game.Tip.Z.I = c.g.N // 무한대에 가까운 |rank|
}
start.Z.I = c.g.N
c.nn = 0
c.activeStack = nil
c.settledStack = nil

@ 탐색을 마치면, |cur_node|가 이끈 팀(뿌리면 |start|)의 이웃 가운데 |goal|에 닿는
것마다 자식 노드를 만들어 계층 $h$에 넣는다. 조건 |untagged==nil|이면 그 정점에서
|goal|에 닿는다는 뜻이고, $h$는 그 정점의 |parent.rank|다.

@<자식마다 새 노드를 만들어 알맞은 계층에 넣는다@>=
base := start
if curNode != nil {
	base = curNode.game.Tip
}
for a := range base.AllArcs() {
	u := a.Tip
	if u.X.A == nil { // |goal|에 닿는다
		x := newNode(curNode, a.A.I) // |del|
		x.game = a
		c.placeNode(x, u.U.V.Z.I) // $h=$ |parent.rank|
	}
}

@ 스택 |settled_stack|에는 모든 이중 성분이 발견된 것과 반대 차례로 담긴다. 이것이
나중에 각 이중 성분에서 |h| 함수를 셈할 때 우리에게 필요한 차례다. 탐색은
|goal|에서 시작해 |dummy|로 되짚어 올라올 때까지 한 걸음씩 나아간다.

@<이중 성분 계산@>=
func (c *chainer) bicomponentDFS(goal *Vertex) {
	v := goal
	v.U.V = c.dummy // |parent|
	c.makeActive(v)
	for v != c.dummy {
		v = c.explore(v)
	}
	@<|settled_stack|으로 각 정점의 상호 도달 수를 셈한다@>
}

@ @<|settled_stack|으로 각 정점의 상호 도달 수를 셈한다@>=
for c.settledStack != nil {
	v := c.settledStack
	c.settledStack = v.W.V         // |link|
	v.Z.I += v.V.V.U.V.Z.I + 1 - c.g.N // |rank += min.parent.rank + 1 - n|
}

@ 정점 |v|를 활성으로 만든다: 새 |rank|를 주고, 활성 스택에 얹고, |min|을 자기
부모로 둔다.

@<이중 성분 계산@>=
func (c *chainer) makeActive(v *Vertex) {
	c.nn++
	v.Z.I = c.nn          // |rank|
	v.W.V = c.activeStack // |link|
	c.activeStack = v
	v.V.V = v.U.V // |min = parent|
}

@ 현재 정점에서 한 걸음 탐색해 새 현재 정점을 준다. 태그 안 된 호가 있으면 그리로
가고, 없으면 |v|가 성숙해 부모로 되짚어 올라간다.

@<이중 성분 계산@>=
func (c *chainer) explore(v *Vertex) *Vertex {
	a := v.X.A // 첫 태그 안 된 호
	if a != nil {
		@<호 |a|를 태그하고 따라간다@>
		return v
	}
	@<|v|가 성숙했으니 부모로 되짚어 올라간다@>
	return v
}

@ 정점 |u=a.tip|을 이미 봤으면 비트리 호이니 |v.min|만 갱신하고, 못 봤으면 트리 호이니
|u|가 새 현재 정점이 된다.

@<호 |a|를 태그하고 따라간다@>=
u := a.Tip
v.X.A = a.Next // 호를 태그한다
if u.Z.I != 0 {
	if u.Z.I < v.V.V.Z.I {
		v.V.V = u // |min|을 갱신
	}
} else {
	u.U.V = v // |parent|
	v = u
	c.makeActive(v)
}

@ 조건 |v.min==v.parent|이면 |v|와 그 자손들이 |u=v.parent|와 함께 이중 성분을 이룬다.
아니면 |u|에서 |v.min|이 보이게 됐으니 |u.min|을 갱신한다.

@<|v|가 성숙했으니 부모로 되짚어 올라간다@>=
u := v.U.V // |parent|
if v.V.V == u {
	c.reportBicomponent(v, u)
} else if v.V.V.Z.I < u.V.V.Z.I {
	u.V.V = v.V.V // |u.min = v.min|
}
v = u

@ 이중 성분을 찾으면 그 정점들의 |parent| 필드를 다시 매겨, 그 뒤로는 두 정점이
같은 이중 성분에 드는 것이 |parent|가 같을 때, 그리고 그때뿐이게 한다. (이 요령은
{\sc BOOK\_COMPONENTS}에서는 쓰이지 않았지만, 비슷한 알고리즘인
{\sc ROGET\_COMPONENTS}에는 나온다.) 새 부모 |v|가 뒤이은 계산에서 그 이중 성분을
대표하며, 우리는 그것을 |settled_stack|에 얹는다.

필드 |v.rank|도 그 이중 성분의 크기에 알고리즘이 헷갈리지 않을 만큼 큰 상수를 더한
값으로 다시 놓는다. (정점 |u|에는 아직 이 이중 성분으로 들어가는 태그 안 된 호가
남아 있을 수 있으므로, rank를 적어도 |u.min|의 rank만큼은 크게 지켜야 한다.)
|v.min|이 이 이중 성분에 딸린 분리점 |u|임을 눈여겨보라. 나중에 |rank| 필드는
여기서 뿌리까지의 모든 셈값의 합을 담게 된다.

정점 |v|가 |goal|일 때는 아무것도 할 필요가 없다. 자명한 뿌리 이중 성분은 언제나 맨
끝에 나오기 때문이다.

@<이중 성분 계산@>=
func (c *chainer) reportBicomponent(v, u *Vertex) {
	if u == c.dummy {
		return // 자명한 뿌리 성분 (|goal|, |dummy|)
	}
	@<활성 스택에서 |v|와 그 자손들을 떼어 이중 성분으로 정착시킨다@>
}

@ @<활성 스택에서 |v|와 그 자손들을 떼어 이중 성분으로 정착시킨다@>=
var cnt int64 // 떼어 낸 정점 수
t := c.activeStack
for t != v {
	cnt++
	t.U.V = v // |parent|
	t = t.W.V // |link|
}
c.activeStack = v.W.V // |v.link|
v.U.V = v             // |parent|
v.Z.I = cnt + c.g.N   // 참된 성분 크기는 |cnt+1|
v.W.V = c.settledStack
c.settledStack = v

@ 다 끝나면 |cur_node.game.tip|이 |goal| 정점이고, |prev| 고리를 따라 |start|
정점까지 되짚어 갈 수 있다. 그런데 답은 |start|에서 |goal| 차례로 찍는 편이 보기
좋으니, 알고리즘을 아예 반대 방향으로 가게 고쳤어야 했는지도 모르겠다.

하지만 그런 사소한 일로 애태우지는 말자. 연결 리스트의 차례를 바꾸기는 쉽다.
비결은 그저 그 리스트를 스택으로 생각하고 원소를 모두 꺼내 다른 스택에 얹는
것이다. 그러면 새 스택에는 원소가 거꾸로 담긴다.
@<사슬을 찍는다@>=
	var top *node
	for chain != nil {
		t := chain
		chain = t.prev
		t.prev = top // 꺼내서
		top = t      // 다시 얹는다(뒤집힌 차례)
	}
	for v := start; v != goal; {
		a := top.game
		u := a.Tip
		@<점수를 출력한다@>
		v, top = u, top.prev
	}

@ 경기 하나의 점수를 찍는다. 필드 |date|(|B.I|)를 달-날로 옮기고, 두 팀의 이름·별명과
점수를 적는다. 정점 |u|의 점수는 |a.len - a.del|이다.
@<점수를 출력한다@>=
@<경기 날짜를 달과 날로 찍는다@>
fmt.Fprintf(c.out, ": %s %s %d, %s %s %d",
	v.Name, v.Y.S, a.Len, u.Name, u.Y.S, a.Len-a.A.I)
fmt.Fprintf(c.out, " (%+d)\n", top.totLen)

@ 0일은 8월 26일이다. 날짜 |d|를 달 이름과 그달의 날로 옮긴다.
@<경기 날짜를 달과 날로 찍는다@>=
d := a.B.I // |date|
var mon string
var day int64
switch {
case d <= 5:
	mon, day = "Aug", d+26
case d <= 35:
	mon, day = "Sep", d-5
case d <= 66:
	mon, day = "Oct", d-35
case d <= 96:
	mon, day = "Nov", d-66
case d <= 127:
	mon, day = "Dec", d-96
default:
	mon, day = "Jan", 1 // |d=128|
}
fmt.Fprintf(c.out, " %s %02d", mon, day)

@* 터미널 상호작용. 프로그램과 사용자가 주고받는 간단한 대화다. 변수 |dummy|를 한 번
마련하고, 시작 팀과 목표 팀을 물어 사슬을 찾아 찍기를 되풀이한다.

@<터미널 상호작용@>=
func (c *chainer) run() {
	c.dummy = new(Vertex)
	sc := bufio.NewScanner(os.Stdin)
	for {
		fmt.Fprintln(c.out) // 눈에 띄게 빈 줄 하나
		if !c.oneRound(sc) {
			break
		}
	}
}

@ 한 판이다. 시작 팀을 물어 없으면 끝(거짓)을 알리고, 목표 팀을 물어 없거나 시작과
같으면 다시 묻는다. 둘이 다르면 사슬을 찾아 찍는다.

@<터미널 상호작용@>=
func (c *chainer) oneRound(sc *bufio.Scanner) bool {
	for {
		start := c.promptForTeam(sc, "Starting")
		if start == nil {
			return false
		}
		goal := c.promptForTeam(sc, "   Other")
		if goal == nil {
			return true
		}
		if start == goal {
			fmt.Fprintln(c.out, " (Um, please give me the names of two DISTINCT teams.)")
			continue
		}
		@<너비가 0이면 탐욕 알고리즘을, 아니면 계층 탐욕을 써서 사슬을 찾아 찍는다@>
		return true
	}
}

@ 너비 |c.width|가 0이면 탐욕 알고리즘을, 아니면 계층 탐욕을 써서 |start|에서
|goal|로 가는 사슬을 찾는다.
@<너비가 0이면 탐욕 알고리즘을, 아니면 계층 탐욕을 써서 사슬을 찾아 찍는다@>=
var chain *node
if c.width == 0 {
	chain = c.greedy(start, goal)
} else {
	chain = c.stratified(start, goal)
}
@<사슬을 찍는다@>

@ 사용자는 팀 이름을 |games.dat|의 표기 그대로 쳐야 한다. 빈 줄이면 |nil|을
준다. 이름을 못 찾으면 아는 팀 하나를 무작위로 귀띔한다.

@<터미널 상호작용@>=
func (c *chainer) promptForTeam(sc *bufio.Scanner, prompt string) *Vertex {
	for {
		fmt.Fprintf(c.out, "%s team: ", prompt)
		if !sc.Scan() || sc.Text() == "" {
			return nil
		}
		name := sc.Text()
		for v := range c.g.AllVertices() {
			if v.Name == name {
				return v
			}
		}
		fmt.Fprintln(c.out, " (Sorry, I don't know any team by that name.)")
		fmt.Fprintf(c.out, " (One team I do know is %s...)\n",
			c.g.Vertices[c.rng.Unif(c.g.N)].Name)
	}
}

@* 찾아보기.
