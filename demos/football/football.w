...
@ |v|에서 아무 이웃으로나 갈 수 있는 것은 아니다. |v| 자신이나 |start|에서
...
@* 이중 성분 계산. $h$ 함수를 셈하기는 얼마나 어려울까? 이어진 그래프 $G$와 두
...
$G$를 조금 키운 그래프 $G^+$를 생각하자. $v$에만 이웃한 가상 정점 $o$를 더한
것이다. $o$에서 시작하는 깊이 우선 탐색으로 $G^+$의 이중 성분을 구한다고 하자.
...
분리점이다. $u$를 없애면 대개 $B_1$이 더 작은 이중 성분들의 트리로 쪼개지지만,
...
@ |settled_stack|에는 모든 이중 성분이 발견된 것과 반대 차례로 담긴다. 이것이
...
|v.rank|도 그 이중 성분의 크기에 알고리즘이 헷갈리지 않을 만큼 큰 상수를 더한
...
|v|가 |goal|일 때는 아무것도 할 필요가 없다. 자명한 뿌리 이중 성분은 언제나 맨
끝에 나오기 때문이다.
