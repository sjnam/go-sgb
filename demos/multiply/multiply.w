...
명령줄에서 \.{multiply} $m$ $n$ [|seed|]라고 부른다. 인자 $m$과 $n$은 곱할 두 수의
비트 크기다. 인자 |seed|는 $m$비트 수에 무작위로 고른 $n$비트 상수를 곱하는 특수
...
수 $301$은 이렇게 나온다. 정밀도가 $999$비트 미만이고
...
@ 인자는 셋이나 넷이어야 한다. 인자 |m|과 |n|은 \CEE/의 |sscanf("%ld")|처럼 앞쪽
...
@ 함수 |Prod|은 정밀도 1000비트 미만만 다룬다. 너무 작은 크기는 2로 올린다.
@<|m|, |n|을 확인하고 |Prod| 그래프 |g|를 짓는다@>=
if m < 2 {
	m = 2
}
...
함수 |PartialGates|가 남긴 회로가 원래 회로보다 작다는 것은 아래에서 찍어 주는 게이트
...
@ 입력이 |m|(또는 |m+n|)비트에 안 들어가면 알리고 넘어간다. 호출 |xVal.Bit(i)|가
|x|의 |i|번째 비트다.
@<게이트 망으로 곱 |z|를 셈한다@>=
...
@ 함수 |parseArg|는 명령줄 인자에서 앞쪽 정수를 읽는다. 함수 |getNumber|는
