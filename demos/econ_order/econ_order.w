...
부문 |v|로 흐르는 산물의 양에 해당하는 수가 붙는다. 조건 $n<79$이면 관련된 상품들을
합쳐 기본 79부문에서 |n|부문을 얻는다. 조건 $s=0$이면 행 합이 고르게 되도록 합치고,
...
|r|개 시도하게 한다. 조건 $r>1$일 때는 앞선 최선을 개선한 해만 보여 준다.
...
@ 상태를 |solver| 구조체에 담아 패키지 수준 가변 상태를 피한다. 필드 |mat|은
