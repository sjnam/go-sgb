...
가는 호마다 두 성분을 합친다. 정점 |v|가 어느 앞선 낱말과도 이웃하지 않으면
...
@ 두 성분이 합쳐질 때는 작은 쪽의 으뜸을 바꾼다. 정점 |v|가 어떤 앞선 낱말과
...
@ 두 순환 리스트를 하나로 잇는 것은 이음 두 개를 맞바꾸는 일이 전부다. 정점 |u|가
$u\to u_1\to\cdots\to u$라는 고리에, |wm|이 $w\to w_1\to\cdots\to w$라는 고리에
있다고 하자. 필드 |u.Z.V|와 |wm.Z.V|를 서로 바꾸면 |u|는 $w_1$을 가리키고 |wm|은
...
@ 두 보조 함수다. 함수 |relink|는 |from|에서 시작하는 순환 리스트의 모든 정점(과
|from| 자신)의 으뜸을 |to|로 바꾼다. 함수 |printMerge|는 합침을 찍되, 첫 합침 뒤
