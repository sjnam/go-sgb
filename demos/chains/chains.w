...
시간이다(기본 0, 안 함). 탐사가 더 좋은 사슬을 찾으면 그것이 최종 결과가 된다.
...
그 방법은 세 단계다. 행렬을 잡고, 정점과 호를 훑어 채운 뒤, 인접 리스트를 짓는다.
@<서브루틴들@>=
...
@ 주 프로그램에서는 휴리스틱이 찾은 사슬을 현직 하계 삼아, 결과를 찍기 전에
탐사를 건다. 분지한정이 그보다 좋은 사슬을 만났으면 그것이 최종 결과가
된다---그래서 \.{-u}와 \.{-c}를 함께 주면 증명된 최적 사슬을 경기별로 구경할
수 있다.
...
@ 일꾼들을 고루틴으로 풀어놓는다. 홀수 번째 일꾼은 탐험가다.
