...
조건 $p=2$이면 |q|는 다시 $104k+(1,3,9,17,25,27,35,43,49,\allowbreak51,75,81)$ 꼴로
...
@ 함수 |readNum|은 프롬프트를 찍고 한 줄을 읽어 정수를 뽑는다. 파일 끝이거나 정수가
...
@ 함수 |Raman|이 돌려준 오류를 {\sc GIRTH}가 밝히는 문구로 옮긴다.
...
@ 조건 $p>2$이면 정수 사원수 이론으로 |Raman|이 내놓는 그래프의 둘레에 대한 하계를
...
성분 $a_1$, $a_2$, $a_3$이 짝수이면 그것들이 모두 $0$일 수는 없으므로
...
이 요구되는 꼴의 표현이 된다. 수 $p^r-q^2$이 세 제곱수의 합으로 나타낼 수 없는 양의
정수라면, 잘 알려진 Legendre의 정리에 따라 $p^r-q^2=4^ts$이고 $s\=7$ (mod~8)이다.
두 소수 $p$와 $q$가 홀수이므로 $t\ge1$이고, 따라서 $p^r-2q^2$은 홀수다. 수 $p^r-2q^2$이 양의
...
둘레가 $1$이나 $2$일 수도 있다. 인자 |p|가 충분히 크면 이 그래프들에 자기 고리나 중복
간선이 생길 수 있기 때문이다.
...
@* 너비 우선 탐색. 함수 |Raman|이 낸 그래프는 대칭이라, 어떤 정점에서 어느 정점으로도
옮기는 자기동형사상이 있다. 그래서 아무 정점 |v0|에서 시작해 둘레와 지름을 찾을
수 있다. 정점 |v0|에서 거리 |k|에 있는 점들의 연결 목록을 만든다. 유틸리티 필드
