...
간선의 길이는 |alph|이면 두 낱말의 알파벳 거리다. |freq|이면 |u|에서 |v|로
가는 호는 |v|의 빈도 비용을 갖지만, |v|에서 |u|로 되돌아오는 호는 $20$을
갖는다. |u|는 사전에 없는 낱말이라 빈도를 물을 수 없기 때문인데, 사전에 있는
...
드러나기는 하지만. |FindWord|는 |g|의 낱말만 알기 때문에, |start|와 |goal|이
서로 이웃이면서 둘 다 원래 그래프에 없으면 그 둘을 직접 이어 주지 못한다.
이건 고쳐 두는 게 낫다. 안 그러면 컴퓨터가 멍청해 보일 테니까.

|gg.N|이 |l.g.N+2|라는 것은 두 낱말이 모두 새로 끼워졌다는 뜻이다. 그때 해밍
거리가 $1$이면---곧 정확히 한 자리만 다르면---손수 이어 준다. |plant|은 늘
...
@ 마지막으로 우리가 남긴 자취를 치운다. |gg|의 새 정점에서 |g|의 옛 정점으로
...
@ |aDist|는 두 낱말의 한 자리 |k|에서의 알파벳 거리다. |alphDist|는 그것을 다섯
자리에 걸쳐 더해, 두 낱말이 이웃이든 아니든 알파벳 거리를 잰다---이웃한 낱말만
다루는 |a.Len| 셈과 달리, 어림 함수는 아무 두 낱말 사이의 거리를 물으므로 이런
일반형이 필요하다.

|hammDist|는 두 낱말의 {\sl 해밍 거리\/}, 곧 서로 다른 글자 자리의 개수다.
