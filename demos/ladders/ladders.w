...
\.{-a}는 이웃한 낱말 사이를 한 단위 떨어진 것으로 보는 대신 {\sl 알파벳
거리\/}로 잰다. 이를테면 \.{words}에서 \.{woods}까지의 알파벳 거리는 $3$인데,
알파벳에서 \.r이 \.o보다 세 자리 뒤이기 때문이다.

\.{-f}는 가장 흔한 낱말에 값 $0$을, 가장 드문 낱말에 값 $16$을 매기고, 그
사이 빈도의 낱말에는 $0$과 $16$ 사이의 값을 매긴다. 그러면 찾아낸 낱말
사다리는 이 잣대로 잰 총비용이 가장 작은 것이 된다. 겪어 보면 \.{-f} 옵션이
가장 ``친근하고'' 가장 마음에 와닿는 사다리를 낳는 경향이 있다.

\.{-h}는 목적 낱말에 가까운 낱말에 우선권을 주어 탐색의 초점을 좁히려 든다.
좀 더 정확히 말하면, 어림 함수 $\\{hh}(v)$를 써서 이웃한 낱말 사이의 거리를
고친다. 여기서 $\\{hh}(v)$는 {\sl 다섯 글자 조합이 모두 영어 낱말이었다면\/}
$v$와 목적 낱말 사이에 있을 법한 가장 짧은 거리다. 그런 어림에 대해서는
{\sc GB\_\,DIJK} 모듈이 더 자세히 설명한다. 이 옵션은 \.{-v}와 함께 두고 볼
때 가장 볼만하다.

@ 프로그램은 먼저 시작 낱말을 묻는다. 그냥 \<return>만 치면 프로그램이 끝나고,
그렇지 않으면 다섯 글자 낱말을(대문자 없이) 친 뒤 \<return>을 쳐야 한다.

그다음 목적 낱말을 묻는다. 여기서 그냥 \<return>을 치면 돌아가서 새 시작 낱말을
다시 묻고, 그렇지 않으면 또 다른 다섯 글자 낱말을 대야 한다.

그러면 프로그램은 시작 낱말에서 목적 낱말까지 최적의 낱말 사다리를 찾아 보여
준다---한 번에 한 글자씩만 바꾸어 가는 길이 있다면 말이다. 그러고 나면 또 다른
시작 낱말로 처음부터 다시 해 볼 기회가 온다.

시작 낱말과 목적 낱말이 프로그램의 ``아는'' 낱말 그래프에 들어 있어야 하는 것은
아니다. 그 둘은 그래프에 임시로 더해졌다가, 새 시작 낱말과 목적 낱말이 주어질
때마다 도로 빠진다. (그래서 \.{sturm}에서 \.{drang}으로도 갈 수 있다, 그 둘이
영어가 아닐지라도.) \.{-f} 옵션을 쓰고 있다면, 목적 낱말이 프로그램의 사전에
없을 때 그 값은 $20$이 된다.
...
떼어 수를 파싱한다. 함수 |usage|와 |num|은 잘못된 인자를 만나면 쓰임새를 알리고
빠져나가는 작은 닫힘함수다.
...
그래프 꼴로 지어 준다. 첫 인자 |n|은 담을 낱말의 최대 개수($0$이면 사전 전부),
둘째 인자는 낱말의 무게를 매기는 아홉 자리 벡터, 셋째는 무게의 문턱값, 넷째는
난수 씨앗이다.

무게 벡터를 |nil|로 주면 {\sc GB\_WORDS}의 기본 무게가 쓰여, 가장 흔한 |n|개
낱말이 뽑힌다. \.{-r} 옵션에서는 그러면 안 되므로 아홉 개가 모두 $0$인 벡터를
넘긴다. 그러면 모든 낱말의 무게가 $0$으로 같아져 빈도 정보가 통째로 무시되고,
|n|개가 난수 씨앗에 따라 무작위로 뽑힌다.
...
@ 함수 |Words|가 낳은 그래프의 간선은 원래 길이가 1이므로, 사용자가 |alph|이나 |freq|을
골랐으면 고쳐야 한다. 이웃한 두 낱말이 다른 글자 자리는 각 호의 |A.I| 필드에
적혀 있고(|loc|), 낱말의 빈도는 정점의 |U.I| 필드에 있다(|weight|). 인자 |alph|일
...
@ 함수 |Dijkstra|의 기본 큐는 모든 간선 길이가 1일 때 아주 효율적이다. 그렇지
않으면 길이가 128보다 작을 때 가장 좋은 128열 큐로 바꾼다. 인자 |alph|·|freq|·|heur|
...
경로를 셈하는 일이다. 함수 |Dijkstra|가 그 일을 해내지만, 한 가지 성가신 점은
|start|와 |goal|이 그래프에 없을 수도 있다는 것이다. 그럴 때 우리는 그들을
임시로 끼워 넣는다.

{\sc GB\_GRAPH}의 관례 덕분에, 정점을 |g|에서 빌려 온 새 그래프 |gg|를 지어
그 증폭(amplification)을 할 수 있다. 그래프 |g|는 정점 두 개(실은 네 개)를 더 둘
자리가 있고, |gg|에 새로 생긴 호를 위한 저장 공간은 나중에 가비지 컬렉터가
치운다. 구조체 |ladders|는 그래프와 옵션들을 한데 묶어, 대화의 매 판마다 쓴다.
...
@ 낱말 |start|에서 |goal|까지 최단 사다리를 찾아 찍는다. 증폭 그래프 |gg|를 짓고,
...
@ 함수 |FindWord|는 |g|에 없는 낱말이면 |nil|을 돌려주는데, 그 전에 두 번째 인자를
이웃한 낱말마다 부른다. 함수 |plant|이 바로 그 인자로, 갓 끼운 정점에서 이웃으로
간선을 심는다. 새 정점은 늘 |gg.Vertices[gg.N]|, 곧 지금 끼우는 중인 여분
슬롯이다.

간선의 길이는 |alph|이면 두 낱말의 알파벳 거리다. 인자 |freq|이면 |u|에서 |v|로
가는 호는 |v|의 빈도 비용을 갖지만, |v|에서 |u|로 되돌아오는 호는 $20$을
갖는다. 낱말 |u|는 사전에 없는 낱말이라 빈도를 물을 수 없기 때문인데, 사전에 있는
가장 드문 낱말의 값이 $16$이니 $20$은 ``사전에 없다''는 뜻의 벌점인 셈이다.
그 밖에는 길이가 $1$이다. \CEE/ 원본은 간선의 두 호가 잇달아 할당된다는 것을
이용해 |u->arcs-1|로 짝을 집었지만(|edge_trick|), 우리는 |Partner|가 짝을
또렷이 가리킨다.
...
@ 위 논리에는 창피할 만한 허점이 하나 있다. 사용자가 짐짓 꾀를 부릴 때에만
드러나기는 하지만. 함수 |FindWord|는 |g|의 낱말만 알기 때문에, |start|와 |goal|이
서로 이웃이면서 둘 다 원래 그래프에 없으면 그 둘을 직접 이어 주지 못한다.
이건 고쳐 두는 게 낫다. 안 그러면 컴퓨터가 멍청해 보일 테니까.

필드 |gg.N|이 |l.g.N+2|라는 것은 두 낱말이 모두 새로 끼워졌다는 뜻이다. 그때 해밍
거리가 $1$이면---곧 정확히 한 자리만 다르면---손수 이어 준다. 함수 |plant|은 늘
|gg.Vertices[gg.N]|을 새 정점으로 보므로, |vv|를 아직 안 끼운 척 |gg.N|을 잠깐
하나 줄였다가 도로 늘린다.
...
@ 마지막으로 우리가 남긴 자취를 치운다. 그래프 |gg|의 새 정점에서 |g|의 옛 정점으로
가는 호는 지우기 쉽다. 옛 정점에서 새 정점으로 가는 호를 지우는 것이 조금 더
까다롭다---간선의 두 호는 짝지어 다니고, 심을 때 짝을 옛 정점의 호 리스트 맨
앞에 밀어 넣었으므로, 새 정점을 심은 차례의 역순으로 훑으며 맨 앞을 하나씩
떼어내면 된다. 이 고리는 시작 낱말과 목적 낱말이 둘 다 |g|가 아니라 |gg|에만
있을 때 그 둘 사이의 호도 제대로 지운다.

\CEE/ 원본은 여기서 |gb_recycle(gg)|를 불러 |gg->data| 블록을 놓아주었다
(|g->data|는 남는다). \GO/에서는 |gg|를 가리키는 것이 아무것도 남지 않는
순간 가비지 컬렉터가 그 몫을 알아서 치우므로 그런 호출이 없다.
...
@ 함수 |aDist|는 두 낱말의 한 자리 |k|에서의 알파벳 거리다. 함수 |alphDist|는 그것을 다섯
자리에 걸쳐 더해, 두 낱말이 이웃이든 아니든 알파벳 거리를 잰다---이웃한 낱말만
다루는 |a.Len| 셈과 달리, 어림 함수는 아무 두 낱말 사이의 거리를 물으므로 이런
일반형이 필요하다.

함수 |hammDist|는 두 낱말의 {\sl 해밍 거리\/}, 곧 서로 다른 글자 자리의 개수다.
...
@ 함수 |promptForFive|는 정확히 다섯 개의 소문자를 엔터와 함께 받을 때까지 조른다.
