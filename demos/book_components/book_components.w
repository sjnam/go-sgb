...
상태를 피한다. 필드 |book|은 |Book| 그래프인지(2글자 코드가 있는지) 여부다.
...
필드 |rank|는 유틸리티 필드 |Z.I|에 둔다.

@ 활성 정점들은 늘 방향 있는 트리를 이루며, 그 호는 원래 그래프의 호 가운데
일부다. 정점 |u|에서 |v|로 가는 트리 호는 |v.parent=u|로 나타낸다. 활성 정점은
...
필드 |untagged|는 유틸리티 필드 |X.A|에 두며, |Arc|를 가리키거나 |nil|이다.
...
지금은 이 정의들을 그냥 믿어 두자. 곧 다 환해질 것이다. 필드 |min|은 유틸리티 필드
|V.V|에 둔다.
...
된다. 정점 |u|에서 |v|로 가는 호가 성숙할 때 고쳐야 하는 것은 |u.min| 하나뿐임을
눈여겨보라. 다른 어떤 |w.min|도 그대로인데, 갓 성숙한 호에는 성숙한 선행자가
없기 때문이다.

동굴 비유가 상황을 밝혀 준다. 방 |u|에서 방 |v|로 들어왔다고 하자. 방 |v|에서
...
@ 정점 |v|가 트리의 뿌리이면 늘 |v.min==dummy|이므로, 성숙하는 순간 언제나 새 이중
...
@ 정점 |vv|를 뿌리로 하는 깊이 우선 탐색이다. 정점 |v|가 현재 정점이며, |dummy|로
...
@ 태그 안 된 호 |a|가 있으면, |v|에서 |u=a.tip|으로 가는 호를 태그한다. 정점 |u|를
...
@ 정점 |v|의 모든 호가 태그됐으면 |v|는 성숙한다. 조건 |v.min==v.parent|이면 |v|와 그
...
@ 정점 |v|가 성숙한 활성 정점이고 |v.min==v.parent|이며 |u=v.parent|라 하자. 정점 |v|와
...
|v|뿐이다(다른 정점은 모두 |v|의 자손이므로). 정점 |x|를 $H$의 정점 가운데 |u|와도
...
게다가 이중연결성을 잃지 않고서는 $H$에 정점을 더 보탤 수 없다. 정점 |w|가 또 다른
...
정점 |v|의 부모인 정점 |u|가 지금 이중 성분에 드느냐 마느냐에는 자잘한 기술적 문제가
하나 있다. 정점 |u|가 가짜 정점이면, |v|가 외딴 정점이었던 경우가 아닌 한, 우리는
...
@ 정점 |u|가 |dummy|이면 |v|는 한 연결 성분의 마지막 이중 성분(또는 외딴 정점)이다.
...
@ 그렇지 않으면 |u|는 뒤이을 이중 성분에도 나타날 분리점이다. 정점 |v|부터 스택을
...
간선 $u\dash v$와 $w\dash x$가 단순 순환 $C$의 간선이고, $w\dash x$와 $y\dash z$가
단순 순환 $D$의 간선이라 하자. 우리는 $u\dash v$와 $y\dash z$를 함께 담은 단순
순환이 있음을 보이려 한다. 순환 $C$의 정점 $a$와 $b$를 잡아
...
그래프 $G$가 이중연결이고 $x$, $y$가 $G$의 서로 다른 정점이라 하자. 그러면 간선
...
귀납법으로 한다. 여기서 $d$는 거리다. 조건 $k=0$이면 성질~P가 결과를 곧바로 준다.
조건 $k>0$이면 대칭성에 따라 $k=d(u,w)$라 해도 된다. 그러면 $u\dash y$이면서
