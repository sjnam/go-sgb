...
크기 $3\times4$ 직사각형 판의 열두 칸을 정점으로 삼고, 한 칸에서 다른 칸으로 퀸의
...
인자 |piece|가 양수이면, 두 칸 사이의 유클리드 거리가 $\sqrt{|piece|}$일 때만 한
수로 갈 수 있다. 그래서 |piece=1|이면 $(x,y)$에서 $(x,y\pm1)$과 $(x\pm1,y)$로
가는데, 킹과 룩이 둘 다 낼 수 있는 수뿐이라 옛 이슬람 체스에서 와지르라 부르던
말이다. 조건 |piece=2|는 $(x\pm1,y\pm1)$ 네 수로, 킹과 비숍이 공유하는 수다.
조건 |piece=5|는 나이트다. 요정 체스 문헌은 이 값들에 이름을 붙여 두었다---와지르 1,
페르스 2, 다바바 4, 나이트 5, 알필 8, 낙타 10, 얼룩말 13, 기린 17 하는 식이다.

인자 |piece|가 음수이면 그 절댓값에 해당하는 기본 수의 임의의 배수가 허용된다.
조건 |piece=-1|은 $(x,y)$에서 $(x\pm a,y)$나 $(x,y\pm a)$로 가는 룩이고,
...
@ 호의 길이도 |piece|의 부호에 달렸다. 인자 |piece|가 양수면 모든 호의 길이가 1이고,
...
@ 룩 판과 비숍 판의 합집합이 퀸 그래프다. 함수 |Board|의 앞 네 인자는 판의 크기로,
