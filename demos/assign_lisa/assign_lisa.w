...
행렬은 $m$행 $n$열이다. 조건 $m\le n$이면 각 행에서 하나씩, $m\ge n$이면 각 열에서
...
그림 전체로 보면 어두운 편이라 위쪽 눈금이 남아돈다. 인자 |d1|을 미소는 100000,
...
행렬의 $0$들이다. 행렬 $A$의 두 $0$이 서로 다른 행, 서로 다른 열에 있으면 {\sl
...
것을 보일 수 있다. 조건 $c\ddash r$이면 $c$나 $r$ 가운데 하나는 뽑혔다. 조건 $c\dash r$
...
K\H{o}nig의 절차로 $A$의 $0$들을 최소 개수 $p$개의 줄로 덮는다. 조건 $p<n$이면 아직
...
바뀔 수는 있어도. 행렬 $A$ 아래에 덧댄 가짜 행 $n-m$개는 늘 덮개의 일부로 뽑히므로,
...
$\sigma_k=\min\{a_{k0},\ldots,a_{k(n-1)}\}$으로 둔다. 조건 $m=n$이면 모든 $k$와 $l$에
...
돌아가자. 조건 $m=n$일 때 각 열에서 최솟값을 빼 두면 시작부터 $0$이 많아진다.
...
길을 터 준다. 배열 |row_dec|와 |col_inc|를 고쳐 새 0을 들인다.
