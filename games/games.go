...
func Ap(v *graph.Vertex) int64          { i, _ := v.U.(int64); return i }
func Upi(v *graph.Vertex) int64         { i, _ := v.V.(int64); return i }
func Abbr(v *graph.Vertex) string       { s, _ := v.X.(string); return s }
func Nickname(v *graph.Vertex) string   { s, _ := v.Y.(string); return s }
func Conference(v *graph.Vertex) string { s, _ := v.Z.(string); return s }

// Arc utility-field accessors.
func Venue(a *graph.Arc) int64 { i, _ := a.A.(int64); return i }
func Date(a *graph.Arc) int64  { i, _ := a.B.(int64); return i }
...
type teamNode = sort.Node[teamData]
...
func Games(n, ap0Weight, upi0Weight, ap1Weight, upi1Weight, firstDay, lastDay, seed int64) (*graph.Graph, error) {
	...
	sorted := sort.LinksSort(&nodeBlock[MaxN-1], rng)
	vi := int64(0)
	for j := 127; j >= 0; j-- {
...
func addGame(g *graph.Graph, u, v *graph.Vertex, su, sv, ven, today int64) {
	// Ensure u < v by pointer address (matches C convention).
	if graph.VertexIndex(g, u) > graph.VertexIndex(g, v) {
		u, v = v, u
		su, sv = sv, su
		ven = HOME + AWAY - ven
	}
