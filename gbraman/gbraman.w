...
@ 함수 |Raman(p,q,typ,reduce)|는 각 정점의 차수가 |p+1|인 무향 그래프를 짓는다.
정점 수는 |typ=1|이면 |q+1|, |typ=2|이면 ${1\over2}q(q+1)$, |typ=3|이면
${1\over2}(q-1)q(q+1)$, |typ=4|이면 |(q-1)q(q+1)|이다. 그래프는 |typ=4|일
때만 이분이다.

인자 |p|와 |q|는 서로 다른 소수여야 하고 |q|는 홀수여야 한다. 여기에 더해 제약이
몇 가지 붙는다. 조건 |p=2|이면 |q|가 $q\bmod8\in\{1,3\}$이면서
$q\bmod13\in\{1,3,4,9,10,12\}$를 만족해야 한다---이 조건 하나로 모든 소수의
4분의 1쯤이 걸러진다. 또 |typ=3|이면 |p|가 |q|를 법으로 하는 이차 잉여라야
한다. 다시 말해 $x^2\=p\pmod q$인 정수 $x$가 있어야 한다. 조건 |typ=4|이면
거꾸로 |p|가 이차 잉여가 아니라야 한다.

@ 인자 |typ=0|을 주면 절차가 허용되는 가장 큰 |typ|(3이나 4)을 스스로 고르고, 고른
값이 그래프 |ID| 문자열에 적힌다. 예컨대 |typ=0|, |p=2|, |q=43|이면 2가 43을
...
인자 |q|의 최댓값은 46337이다. 제곱이 $2^{31}$보다 작은 가장 큰 소수다. 물론 이 값은
type 1 그래프에나 쓸 만하다.

인자 |reduce|가 0이 아니면 자기 고리와 중복 간선을 없앤다. 그러면 위에서 말한 바와
...
@ 타입 |builder|는 \CEE/의 정적 전역(모듈로 |q| 산술표, 생성원 표와 그 개수)을 담아
패키지 수준 가변 상태를 피한다. 타입 |quaternion|은 사원수 하나와 그 켤레(역원)의
인덱스를 담는다.
...
@ 함수 |Raman|의 뼈대다. 표를 세우고, |typ|을 고르거나 검사해 정점 수 |n|을 정한
...
대신, |q|가 정점 수보다 훨씬 작으니 표를 통째로 만든다. 값 |qSqr[k]|는 $k^2$,
|qSqrt[k]|는 $\sqrt{\mathstrut k}$(이차 잉여가 아니면 $-1$), |qInv[k]|는 |k|의
역원이다.

@ 함수 |prepareTables|는 세 표를 세운다. 인자 |q|가 소수인지는 원시근을 찾다가 은근히
...
@ 원시근이면 그 거듭제곱이 모든 것을 낳는다. 인자 |q|가 소수가 아니면 안쪽 반복이
|k>=q|로 끝난다.
...
@ 원시근을 찾으면 역원을 모두 쉽게 낳는다. 값 |qInv[0]=q|로 두어 $\infty$를 안에서
나타낸다.
...
@ 함수 |chooseType|은 |typ|을 고르거나 검사하고 정점 수 |n|과 |nFactor|를 정한다.
인자 |p=2|일 때의 |q| 조건은 이차 상호 법칙에 따라 $\sqrt{-2}$와 $\sqrt{13}$가
모듈로 |q|로 있다는 것과 같다.
...
@ type 1은 |X.I|에 일련번호를 두되 |q|로 $\infty$를 나타낸다. 필드 |Y.I|는 안 쓰므로
끈다.
...
@ 조건 |p=2|일 때는 Ramanujan 그래프를 낳는 특별한 생성 행렬 $\pi_0$, $\pi_1$,
...
$i^2=j^2=k^2=ijk=-1$을 따른다. 사원수 $\alpha=a+A$라 쓰자---$a$는 ``스칼라'' $a_0$이고
$A$는 ``벡터'' $a_1i+a_2j+a_3k$다. 그러면 사원수 $\alpha=a+A$와 $\beta=b+B$의
곱은
$$(a+A)(b+B)=ab-A\cdot B+aB+bA+A\times B$$
로 나타낼 수 있다. 곱 $A\cdot B$와 $A\times B$는 여느 벡터의 내적과 외적이다.

@ 사원수 $\alpha=a+A$의 켤레는 $\overline\alpha=a-A$이고,
$\alpha\overline\alpha=a_0^2+a_1^2+a_2^2+a_3^2$이다. 이 중요한 값을
$N(\alpha)$, 곧 $\alpha$의 노름이라 부른다. 등식 $N(\alpha\beta)=N(\alpha)N(\beta)$임을
확인하기는 어렵지 않다. 기본 항등식
...
@ 함수 |computeGenerators|는 |gen| 표를 채운다. 인자 |p|가 소수가 아니면 |p+1|개보다 많은
...
채로 나온다. 그러니 그 여덟 중 하나를 골라야 한다. 등식 $a_0^2+a_1^2+a_2^2+a_3^2=p$
라 하자.

값 |p| mod 4가 1이면 $a$들 가운데 정확히 하나가 홀수다. 그것을 $a_0$이라 부르고
양의 부호를 준다. 값 |p| mod 4가 3이면 정확히 하나가 짝수다. 그것을 $a_0$이라
...
@ 조건 |a>0|이고 $0<b<c<d$이면 $\{b,c,d\}$를 여섯 가지로 치환하고 부호를 여덟 가지로
붙여, 노름이 같은 사원수의 부류 48개를 얻는다. 이런 일은 이를테면 $p=71$이고
$(a,b,c,d)=(6,1,3,5)$일 때 일어난다. 조건 |a=0|이거나 $b=0$이거나 $b=c$이거나
$c=d$이면 더 적게 나온다.

사원수에 대응하는 행렬의 역행렬은 그 켤레 사원수에 대응하는 행렬이다. 그러므로
생성 행렬 $\pi_k$가 자기 자신의 역이 될 필요충분조건은 그것이 |a=0|인 사원수에서
나온 것이라는 점이다. 함수 |deposit|은 새 사원수와 그 켤레를 함께 생성원 표에 넣는다.
...
@ 조건 $b<c$이면 $\{b,c\}$를 맞바꾼 치환들이 더 나온다.
...
@ 조건 $c<d$이면 $d$를 셋째 자리로 올린 치환들이 더 나온다.
...
@ 조건 |p=2|일 때는 Patrick Chiu가 찾아낸 다음 세 생성 행렬이 알맞다:
@^Chiu, Patrick@>
...
@* 간선 잇기. 표 |gen|이 정한 순열들로 호와 그 역호를 만든다. 호의 |A.I|
필드(|ref|)에 그 호를 낳은 순열 번호를 적어, 대개 각 정점의 간선 목록이 |ref|
오름차순이 되게 한다. 인자 |reduce|가 0이 아니면 자기 고리와 중복 간선을 없앤다.

@ 인자 |reduce|가 0이라도 어떤 순열이 고정점을 가지면 그 정점에 자기 고리(호 두
...
@ 정점 |u|의 첫 호가 이미 |ref==kk|이면 (|kk=k|인 2-사이클을) 이미 처리한 것이니
건너뛴다. 인자 |reduce|이면 |u|로 가는 호가 이미 있는지 살펴 있으면 건너뛴다.
...
@ 네 |typ|의 정점 수 공식도 확인한다. 식 |q+1|, ${1\over2}q(q+1)$,
${1\over2}(q-1)q(q+1)$, $(q-1)q(q+1)$이다. 조건 |p=3|, |q=13|이면 3이 13을 법으로
