...
type charData struct {
	...
	vert *graph.Vertex // assigned vertex (nil if not selected)
}

type charNode = sort.Node[charData]
...
func Desc(v *graph.Vertex) string     { s, _ := v.Z.(string); return s }
func InCount(v *graph.Vertex) int64   { i, _ := v.Y.(int64); return i }
func OutCount(v *graph.Vertex) int64  { i, _ := v.X.(int64); return i }
func ShortCode(v *graph.Vertex) int64 { i, _ := v.U.(int64); return i }
...
func bgraph(bipartite bool, title string, n, x, firstChap, lastChap, inWeight, outWeight, seed int64) (*graph.Graph, error) {
	...
	sorted := sort.LinksSort(&nodeBlock[characters-1], rng)
