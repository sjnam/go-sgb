...
@ 크기 |M=|\thinspace|m1-m0|행과 |N=|\thinspace|n1-n0|열의 입력을 |m|행 |n|열의
...
@ 인자 |m|·|n|·|d|·|m1|·|n1|·|d1|을 0으로 주면 기본값이 자동으로 채워진다.
인자 |m1|이 0이거나 360보다 크면 360으로, |n1|이 0이거나 250보다 크면 250으로
바뀐다. 그러고 나서 |m|이 0이면 |m1-m0|으로, |n|이 0이면 |n1-n0|으로 바뀐다.
인자 |d|가 0이면 255가 되고, |d1|이 0이면 $255(|m1|-|m0|)(|n1|-|n0|)$이 된다.
이 치환이 모두 끝난 뒤의 매개변수는 반드시
$$\hbox{|m0<m1|,\qquad |n0<n1|,\qquad |d0<d1|}$$
을 만족해야 한다. (|d0|만은 기본값 치환의 대상이 아니다.)

@ 보기를 몇 가지 들어 보자. 호출 |Lisa(0,0,0,0,0,0,0,0,0,dir)|은
...
호출 |Lisa(250,250,255,0,250,0,250,0,0,dir)|은 그림 위쪽에서 잘라 낸 정사각 배열을
준다. 아래쪽 모나리자의 두 손은 빠진다.

호출 |Lisa(36,25,25500,0,0,0,0,0,0,dir)|은 원래 자료의 $10\times10$ 부분정사각형을
더해 만든, 값이 $[0,25500]$ 범위인 $36\times25$ 배열을 준다.

@ 호출 |Lisa(100,100,100,0,0,0,0,0,0,dir)|은 값이 $[0,100]$ 범위인 $100\times100$
...
비 |(m1-m0)/m|과 |(n1-n0)/n|이 같으면 |Lisa|의 출력은 ``정사각 픽셀''을
나타낸다. 비 |(m1-m0)/m|이 더 작으면 이 출력으로 만든 하프톤이 가로로 눌리고,
더 크면 세로로 눌린다.
...
호출 |Lisa(1000,1000,255,0,250,0,250,0,0,dir)|은 원래 이미지 위쪽에서 백만 개의
...
@ 타입 |Matrix|는 |Lisa|가 돌려주는 값이다. 필드 |Pix|는 길이 $|M|\times|N|$인 픽셀
...
@* Lisa 서브루틴.
함수 |Lisa(m,n,d,m0,m1,n0,n1,d0,d1,dir)|은 |dir| 디렉터리의 \.{lisa.dat}에서
$[|m0|,|m1|)$행·$[|n0|,|n1|)$열의 직사각형 조각을 뽑아, 값이 $[0,d]$ 범위인
$|m|\times|n|$ 정수 행렬을 만든다. 인자 |m|·|n|·|d|·|m1|·|n1|·|d1| 가운데 0으로
주어진 것은 기본값으로 바뀐다.
...
수 $n$을 2진법으로 쪼개 비트를 스택에 밀어 넣었다가 되꺼내면서, 몫과 나머지를
...
@ 인자 |d0|는 기본값을 채우지 않는다. 인자 |d1|만 0일 때 $255MN$으로 바뀐다. 식별
...
변수 |kappa|는 |inRow|에 담긴 입력 픽셀의 거대 행렬 속 아래 경계, |kap|은 아직 쓰지
않은 첫 거대 행, |nextKap|은 출력 행 |k|의 아래 경계다. 배수 |fac|은 지금 입력 합을
몇 곱해 복제할지를 나타내는 인자다.
...
@ 안쪽 고리다. 변수 |lambda|는 |curPix|의 입력 픽셀이 차지하는 거대 열의 오른쪽
...
포인터 산술을 |inRow|의 인덱스 |p|로 옮긴다. 변수 |dd|는 고리 밖에 두어야
...
함수 |PlaneLisa(m,n,d,m0,m1,n0,n1,d0,d1,dir)|은 |Lisa|가 낸 디지털화에 대응하는
...
표 $f$의 주된 구실은 한 영역의 맨 위·왼쪽 픽셀을 가려내는 것이다. 자리
...
않는다. 값 |f[j]|를 |j|로 되돌리는 안쪽 고리 탓에 최악의 경우 $mn^2$ 시간이 들 수
...
@ 값 |a[ap]|이 음수이면 이 자리가 새 영역의 시작이므로 새 정점을 만들고, 아니면
...
@ 함수 |adjac|은 두 정점이 아직 이웃이 아니면 이웃으로 만든다. 두 곳에서 부르므로
...
함수 |BiLisa(m,n,m0,m1,n0,n1,thresh,c,dir)|은 |Lisa|가 낸 $m\times n$ 디지털화에
대응하는 이분 그래프를 짓는다. 인자 |m0|·|m1|·|n0|·|n1|은 앞서 설명한 대로
...
@ 함수 |Lisa|를 |d=65535|로 불렀으므로 이웃 판정이 간단하다. 행 정점은 |"r0"|,
...
@ 호출 |plane_lisa(100,100,50,1,300,1,200,2975050,11900200)|은 정점 2452개·호
...
@ 함수 |BiLisa|는 |sample.correct|에 없으므로, 짜임새만 확인한다. 미소 영역의
...
@ 이름난 두 부분의 크기를 확인한다. 변수 |Smile|은 원본 주석대로 $16\times32$이고,
