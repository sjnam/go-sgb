...
func SectorTotal(v *gbgraph.Vertex) int64     { i, _ := v.Y.(int64); return i }
func SICCodes(v *gbgraph.Vertex) *gbgraph.Arc { a, _ := v.Z.(*gbgraph.Arc); return a }

// Arc utility-field accessor.
func Flow(a *gbgraph.Arc) int64 { i, _ := a.A.(int64); return i }
...
type econState struct {
	...
	vertIndex [MaxN + 2]*gbgraph.Vertex // vertIndex[1..81]: vertex assigned to SIC code
}
...
func Econ(n, omit, threshold, seed int64) (*gbgraph.Graph, error) {
	rng := gbflip.New(seed)
...
func (s *econState) growRandomSubtree(l int64, rng *gbflip.RNG) {
	s.nAt(0).tag = l
...
func (s *econState) stochasticallyDivide(p, pl, pr *econNode, l int64, rng *gbflip.RNG) {
	klo := l - pr.table[0]
	if klo < 1 {
		klo = 1
	}
...
func (s *econState) putArcs(g *gbgraph.Graph, n, omit, threshold int64) {
